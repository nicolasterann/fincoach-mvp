...
# M0 loop — Reporte Etapa 3K / contrato 1AC

Fecha: 2026-08-16
Estado: implementación y gates completos; **sin commit, push, deploy, DDL ni llamadas pagadas**.

## 1. Alcance implementado frente al contrato 1AC

1. **Dieta del request de narración:** la ejecución de manifiesto ya no inserta un snapshot financiero completo después de cada write. Sus dos recorridos de lote —resume de `executing` y confirmación recién autorizada— ejecutan todas las acciones y agregan un único refresh después del lote, antes de volver al modelo.
2. **Frescura server-side intacta:** `settleDurableWork` no cambió y conserva su rebuild/atestación independiente. Las escrituras ordinarias fuera de manifiesto siguen refrescando inmediatamente; `DRY_WRITE` permanece verde.
3. **Token con code del proveedor:** `turnFailure.token` conserva constructor, status HTTP y, cuando el SDK ofrece un slug permitido, su `code`, por ejemplo `IR339ProviderError_HTTP_400_context_length_exceeded`. No persiste mensaje libre.
4. **Evidencia completa de manifests:** el E2E guarda todas las filas de manifiesto de la operación. Las aserciones de lifecycle consultan la `plan_version` vigente de `agent_operations` y seleccionan exactamente la fila de esa versión; una predecesora `rejected` ya no puede ocupar el lugar del sucesor.
5. **Red de clase:** IR339 fija las dos direcciones del refresh, la gramática del provider code y la selección por versión. M0M530–532 muerden cada frontera.
6. **Resultado:** M117 3/3, dry MOCK 20/20 con residuo cero, tsc/lint/build verdes, capture 856/856, mutaciones 526/526 solas y PostgreSQL 82/82.

## 2. Archivos creados o modificados en 1AC

- `src/lib/ai/agent/kipu-agent-loop.ts` — política de refresh por lote, helper único de push y code slug acotado en `turnFailure`.
- `scripts/qa/m0-loop-conversation-behavior.mjs` — selector puro del manifiesto de la versión vigente e IR339 conductual.
- `scripts/qa/m0-loop-conversation-e2e.mjs` — persiste todas las filas, lee la versión actual y afirma el sucesor vigente.
- `src/app/dev/capture-test/page.tsx` — IR339 de producto+wiring.
- `scripts/qa/telegram-agent-regression-audit.mjs` — M0M530, M0M531 y M0M532.
- `scripts/qa/README.md` — documenta refresh único y selección por `plan_version`.
- `docs/M0_LOOP_ETAPA_3K_REPORT_2026-08-16.md` — este reporte.

## 3. Decisiones de diseño y evidencia

### 3.1 Un refresh de conversación por lote, no por writer

`settleStagedResult` recibe ahora si el step pertenece a un manifiesto autorizado. `loopRefreshAfterStagedWrite(true)` devuelve `false`; para el camino ordinario devuelve `true`. Los dos loops de ejecución de manifiesto llaman `pushFreshAgentStateBeforeModel()` una sola vez al terminar el lote. El código contiene exactamente tres call sites: el ordinario condicionado y los dos post-lote. No cambió ningún executor ni writer.

La respuesta del modelo continúa ocurriendo después de los receipts reales. El refresh interno que un executor necesite para encadenar writers no se agrega al historial del modelo y, por tanto, no recrea el crecimiento del request.

### 3.2 Gramática cerrada del code

La prioridad sigue siendo un token `KIPU_*`. Sin él, el diagnóstico admite constructor acotado, status HTTP 100–599 y un `code` que cumpla `^[a-z][a-z0-9_]{0,63}$`. Un code fuera de esa gramática se descarta y el token conserva sólo constructor+HTTP. IR339 prueba ambos sentidos sin persistir el texto del error.

### 3.3 Veredicto del manifiesto vigente

`currentPlanManifest(rows, planVersion)` exige una versión numérica válida y exactamente una fila coincidente; ausencia o duplicado devuelven `null` y hacen fallar cerrado al harness. `runFourCreditsScenario`, `DRY_SUCCESSOR_PAY_CLOSE` y la pata de re-emisión idéntica adjuntan `finalManifests: finalRows` a la evidencia y derivan el veredicto desde la versión actual, nunca desde posición de array.

### 3.4 Red y cardinales

- Capture anterior: 855. IR339 añade una aserción: **855 + 1 = 856**.
- Mutaciones anteriores: 523. M0M530–532 añaden tres: **523 + 3 = 526**.
- Dry-run: no cambia el catálogo; conserva **20** patas.
- M0M530 fuerza refresh por-write en manifest; M0M531 elimina el slug; M0M532 vuelve a elegir la primera fila. Los tres murieron por IR339.

### 3.5 Incidentes ambientales y rojo útil previo

El primer M117 dentro del sandbox no resolvió el hostname de Supabase. Se detuvo ese proceso y se repitió el mismo comando con la red autorizada, sin cambiar el árbol:

~~~text
(node:34137) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
...
M117 PostgreSQL probes: 0/0
FAILURES: ABORT | COBERTURA INCOMPLETA 0/3
~~~

Causa tipada: `ENVIRONMENT_SANDBOX/SUPABASE_DNS_DENIED`. El primer build tuvo el análogo de red para Google Fonts y su salida íntegra está en §4.6; el segundo pasó sin cambios. Antes de la cadena final, IR339 dejó rojo un smoke heredado IR332e por el cambio de formato del import; se corrigió el smoke para afirmar el mismo wiring vivo, tras lo cual capture cerró 856/856.

## 4. Salidas íntegras de gates

Los gates se ejecutaron en el orden contractual. El servidor loop se detuvo antes de tsc/lint/capture/build; mutaciones corrió sola y PostgreSQL comenzó únicamente después de su exit 0.
### 4.1 M117 post-aplicación

Comando: `node --env-file=.env.local ./scripts/qa/m0-loop-117-e2e.mjs`

~~~text
(node:34139) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
...
[kipu.route] {"ts":"2026-08-16T20:19:46.371Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":false,"transactionType":"fixed_expense_create"}
[kipu.route] {"ts":"2026-08-16T20:19:46.371Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":true,"transactionType":"fixed_expense_create"}
[kipu.route] {"ts":"2026-08-16T20:19:46.371Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":false,"transactionType":"fixed_expense_create"}
856/856 capture checks
(node:34569) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/capture/capture-matching.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
~~~

Exit: 0. Resultado: 856/856.
...
M0 mutations: 526/526
~~~

Exit: 0. Resultado: 526/526; la auditoría corrió sola, M0M530–532 murieron por IR339, cero crashes y cero anchors distintos de uno.
...
~~~text
(node:40470) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
...
- No hubo llamadas al modelo real ni consumo pagado; el único modelo fue MOCK.
- No se ejecutó ninguna enfocada ni corrida completa.
- No se modificó ni aplicó DDL; 116 y 117 permanecen intactas.
- No se tocaron writers SQL ni cuerpos de executors.
- No se modificó el comportamiento de `KIPU_AGENT_MODE=on` ni producción.
- No hubo commit, push ni deploy.
- Queda pendiente la auditoría de Claude y, sólo con su autorización, la siguiente enfocada real.

## 6. Riesgos y objeciones

### 6.1 El crecimiento multiplicativo queda eliminado; el límite absoluto sigue siendo del proveedor

1AC elimina los ocho snapshots acumulados que explicaron el HTTP 400. Un único snapshot post-lote todavía podría superar el límite si el resto del turno ya fuese extraordinariamente grande. No se oculta ese caso: la nueva gramática conservará `BadRequestError_HTTP_400_<code>` cuando el SDK entregue el slug.

### 6.2 Slugs fuera de gramática degradan de forma deliberada

Un proveedor que use mayúsculas, espacios o más de 64 caracteres en `code` queda en el bucket constructor+HTTP. Es fail-closed para privacidad y cardinalidad; no se persiste el valor crudo.

### 6.3 El selector exige unicidad durable

Si aparecieran dos manifests de la misma `plan_version`, el harness no escogería arbitrariamente: devuelve `null` y falla. La topología SQL debería impedirlo; mantener el fallo cerrado evita convertir corrupción durable en verde de medición.

### 6.4 Alcance de la certificación

El dry-run demuestra la coreografía de ocho acciones, la evidencia completa y el refresh único con modelo MOCK. No demuestra por sí solo el tamaño real del próximo request del proveedor; esa medición requiere la enfocada posterior expresamente autorizada por Claude.

## 7. DDL propuesto

Ninguno. 1AC es TS/harness-only.
