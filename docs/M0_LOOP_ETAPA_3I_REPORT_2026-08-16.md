...
# M0 loop — Reporte Etapa 3I / contrato 1AA

Fecha: 2026-08-16  
Estado: implementación y gates completos; lista para auditoría de Claude  
Llamadas pagadas: cero

## 1. Alcance implementado frente al contrato 1AA

1. **Observabilidad de settle:** settleDurableWork identifica la sub-etapa activa como transition, fresh_read, step_verify o manifest_verify. Todo throw de ese bloque se reduce a un diagnóstico acotado; step_verify conserva además stepKey y capability.
2. **Detalle de wrappers:** los wrappers de verify conservan sólo el token server-owned KIPU_*; nunca persisten el mensaje PostgREST arbitrario.
3. **Durabilidad y E2E:** loopDiagnostic del catch exterior lleva settleFailure, cruza el handler hacia metadata durable del mensaje asistente y turnDetail lo imprime antes del cleanup de un fallo.
4. **Reproducción gratis:** se agregó DRY_SUCCESSOR_PAY_CLOSE con cuatro tarjetas, propuesta de cuatro pagos, sucesor de ocho acciones al añadir cuatro cierres, confirmación, ejecución y settle.
5. **Resultado de la reproducción:** **NO reprodujo** POST_WRITE_MANIFEST_SETTLEMENT_UNAVAILABLE. Pasó focalmente 1/1 y volvió a pasar dentro del dry completo 19/19: cuatro pagos exactos, cuatro tarjetas cerradas, manifiesto verified, paridad 8/8 y diagnóstico nulo.
6. **Fix causal:** ninguno. El contrato autorizaba reparar sólo una causa probada por la reproducción. Tocar rebuild, verify, SQL o freshness sin reproducir habría sido especulativo.
7. **Fronteras:** no se modificaron SQL/DDL, writers, executors, el camino envelope/on, 117/1X, cohesión 1Y ni redirects 1Z.

Desviación explícita: la hipótesis pre-registrada rebuild → KIPU_READ_FAILED no se confirmó en MOCK. Tampoco quedó refutada para la ejecución con modelo real: el fixture determinista demuestra que la forma durable de ocho acciones puede asentarse correctamente en este árbol.

## 2. Archivos creados o modificados en 1AA

- src/lib/ai/agent/agent-operation-store.ts — preserva únicamente el token KIPU_* de errores de verify.
- src/lib/ai/agent/kipu-agent-loop.ts — sub-etapas tipadas y diagnóstico acotado de settle en el resultado del turno.
- scripts/qa/m0-loop-conversation-e2e.mjs — turnDetail observable y nueva pata DRY_SUCCESSOR_PAY_CLOSE.
- src/app/dev/capture-test/page.tsx — IR337 conductual y smoke de wiring de observabilidad/reproducción.
- scripts/qa/README.md — cardinal vigente de 19 patas y mención de la nueva cobertura.
- docs/M0_LOOP_ETAPA_3I_REPORT_2026-08-16.md — esta acta.

## 3. Decisiones de diseño y evidencia traza-primero

### 3.1 Diagnóstico bounded por construcción

El wrapper extrae con una gramática cerrada únicamente KIPU_[A-Z_]{2,80}. Si no existe un token server-owned, el settle usa uno de cuatro tokens estáticos: settle_transition_failed, settle_fresh_read_failed, settle_step_verify_failed o settle_manifest_verify_failed. stepKey y capability atraviesan listas permitidas de caracteres y longitud. No se copia texto del usuario ni detalle crudo de base.

### 3.2 Evidencia de la no-reproducción

La pata focal pasó con exit 0. Antes de confirmar, el snapshot financiero fue byte-equivalente al inicial. El sucesor durable contenía exactamente cuatro register_card_payment seguidos de cuatro close_card; luego aparecieron cuatro transacciones debt_payment por 11,11 / 12,22 / 13,33 / 14,44, el saldo bajó 51,10, las cuatro tarjetas quedaron closed y el manifiesto final reportó authorized_count=8, verified_count=8, status=verified. settleDiagnostic=null.

La misma pata volvió a pasar en la corrida completa mostrada en §4.2. Por disciplina traza-primero no se alteró ninguna causa candidata de A19-1.

### 3.3 Red y cardinales

- Capture previo: 853. IR337 agrega una aserción: 853 + 1 = **854**.
- Mutaciones previas: 520. 1AA no agrega mutante porque A19 exige observabilidad + reproducción y no hubo fix causal que mutar: **520**.
- Dry previo: 18. DRY_SUCCESSOR_PAY_CLOSE: 18 + 1 = **19**.
- IR337 prueba ambos sentidos del detalle: KIPU_READ_FAILED se conserva y texto opaco se descarta; prueba además step_verify con identidad, fallback de manifest_verify, el cable durable y turnDetail.
...
~~~text
(node:6901) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
...
### 4.2 Dry-run MOCK loop — 19 patas
...
Dry-run MOCK: ejecuta 19 recorridos representativos y valida estáticamente los 50 contratos del catálogo.
...
loopUsage agregado: {"cachedInputTokens":3400,"calls":85,"inputTokens":8500,"outputTokens":1700}
Judge usage agregado: {"cachedInputTokens":0,"calls":19,"inputTokens":3420,"outputTokens":855}
Paraphrase usage agregado: {"cachedInputTokens":0,"calls":0,"inputTokens":0,"outputTokens":0}
Costo real acumulado: 0.041836 USD
Calidad promedio: 5.00/5
Costo estimado corrida completa: {"baseline":"native loop","basis":"MOCK token telemetry plus a deterministic paraphrase allowance, scaled to the complete catalog","estimatedTurns":108,"estimatedUsd":0.12,"ratesUsdPerMillion":{"coach":{"cached":0.25,"input":2.5,"output":15},"mini":{"cached":0.1,"input":0.4,"output":1.6}},"scenarios":50,"tokens":{"agent":{"cachedInputTokens":9415,"calls":235,"inputTokens":23538,"outputTokens":4708},"judge":{"cachedInputTokens":0,"calls":50,"inputTokens":9000,"outputTokens":2250},"paraphrase":{"cachedInputTokens":0,"calls":1,"inputTokens":677,"outputTokens":736}}}
M0 tres carriles (loop, MOCK): 19/19 duros verdes
~~~

Exit: 0. Resultado: 19/19; residuo de personas cero. El costo impreso es valoración sintética de telemetría MOCK; no hubo consumo facturado.
...
[kipu.route] {"ts":"2026-08-16T15:52:47.348Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":false,"transactionType":"fixed_expense_create"}
[kipu.route] {"ts":"2026-08-16T15:52:47.348Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":true,"transactionType":"fixed_expense_create"}
[kipu.route] {"ts":"2026-08-16T15:52:47.348Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":false,"transactionType":"fixed_expense_create"}
...
Exit: 0. Resultado: 520/520; catálogo de mutaciones sin cambios en 1AA, cero crashes y cero anchors distintos de uno.
...
~~~text
(node:11020) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
...
Repetición documental posterior, primer intento dentro del sandbox. Comando idéntico:

~~~text
(node:11019) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
...
Exit: 1. Causa tipada: `ENVIRONMENT_SANDBOX/SUPABASE_DNS_DENIED`. Sin cambios de árbol, la repetición autorizada produjo el transcript 82/82 inmediatamente anterior, exit 0.


## 5. Qué no se hizo y qué queda pendiente

- No se hicieron llamadas pagadas; toda inferencia de esta etapa fue MOCK.
- No se aplicó un fix causal porque el rojo no reprodujo.
- No se hizo la enfocada real ME17 + REAL_FOUR_CREDITS; espera el OK de Claude.
- No se creó, cambió ni aplicó DDL.
- No se tocaron writers SQL ni lógica de executors.
- No se hizo commit, push ni deploy.
- No se escribió contra la cuenta real del founder; los fixtures fueron personas marcadas y terminaron con residuo cero.
- No se modificó producción ni el camino KIPU_AGENT_MODE=on.

## 6. Riesgos y objeciones

### 6.1 No-reproducción local

El diferencial real no quedó aislado. El fixture MOCK reproduce la forma durable de ME17, pero no necesariamente la coreografía estocástica previa o posterior del modelo real. La observabilidad nueva es precisamente la condición para que la próxima enfocada identifique transition, fresh_read, el step exacto o manifest_verify sin volver a colapsar a unavailable.

### 6.2 Diagnóstico durable

El diagnóstico vive en assistantMetadata.loopDiagnostic.settleFailure, persistido por el camino normal del mensaje asistente. Si el proceso cayera antes de persistir el mensaje, seguirían existiendo steps/manifiesto, pero no este diagnóstico de turno; 1AA cubre los fallos retornados por settleDurableWork, no un crash de proceso.

### 6.3 Incidentes ambientales de gates

El primer build dentro del sandbox falló sólo al descargar Google Fonts; el mismo árbol pasó fuera del sandbox. Después de la cadena oficial, una repetición documental de PostgreSQL falló dentro del sandbox por ENOTFOUND hacia Supabase (0/0); sin cambiar archivos se repitió con red autorizada y pasó 82/82. Ninguno fue fallo de producto.

No hay objeción arquitectónica ni bloqueo que requiera DDL.

## 7. DDL propuesto

Ninguno. 1AA es TS/harness/IR y la reproducción no probó una causa que exigiera migración.
