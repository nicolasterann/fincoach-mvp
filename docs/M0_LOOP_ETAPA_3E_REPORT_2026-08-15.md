...
Fecha: 2026-08-15  
Estado: implementación terminada; migración 117 preparada y **NO APLICADA**; lista para auditoría pre-aplicación de Claude.  
Árbol: worktree local no comiteado, sin push, deploy ni aplicación de DDL.  
Baseline de producción: `KIPU_AGENT_MODE=on` intacto; esta ronda sólo modifica el camino `loop`, harnesses y una exportación pura sin cambio de conducta.

## 1. Alcance implementado frente al contrato 1W

1. **Barrido definitivo de writers.** Se revisaron todas las referencias SQL a `plan.actions`. La única función que además es un writer económico directo alcanzable por el loop es `public.kipu_apply_debt_proceeds`. Las referencias restantes pertenecen a resume/save/verify del envelope, al registro de manifiesto legacy o a `kipu_preflight_agent_operation_step`, que el loop no usa por contrato A2-2.3. La evidencia exacta está en §4.10.
2. **Migración 117 preparada, no aplicada.** `CREATE OR REPLACE public.kipu_apply_debt_proceeds(jsonb)` conserva ownership, lease, plan version, step/capability, argumentos, replay, dedupe y writer. Para `plan={"mode":"loop"}` autoriza sólo el step stageado exacto: fingerprint `md5(arguments::text)`, marcador económico `economic_event/capability_catalog` y espejo exacto dentro de un manifiesto `loop_staged` ya `executing`, autorizado por una delivery distinta. El predicado legacy `plan.actions[].effects.classification='debt_proceeds'` queda intacto en la rama no-loop.
3. **Topología segura.** La función sigue `SECURITY DEFINER`, fija `search_path=public, pg_temp`, conserva owner/revokes/grant y termina con un DO-block que prueba los dos branches y la topología. No se usa `digest`: el contrato vigente de staging usa el `md5` incorporado, por lo que `extensions.digest` no aplica.
4. **Canonicalización de target.** La consolidación viva resuelve nombre o id mediante el mismo resolver tipado que ya usa `executeTool`; ambos producen una clave `id:<uuid>`. No se añadió un router, regex ni interpretación nueva.
5. **Sondas post-aplicación M117.1–M117.3.** El harness nuevo prueba: propuesta→segunda delivery→authorize→begin→write exacto de caja+deuda→verify; replay idempotente; y rechazo del plan envelope sin `classification=debt_proceeds`. Siembra `chat_messages`, `user_engagement.timezone` e `is_currency_default`, usa sólo RPCs para tablas protegidas y comprueba residuo por PK real.
6. **`DRY_BORROWED_LINK` asciende a E2E.** Ahora confirma el manifiesto, exige un único adjustment de 83,86, refs tipados poseídos, step/manifest verified, y deltas exactos de cuenta y deuda. El freno pre-117 IR332g fue retirado.
7. **Red.** IR333a fija nombre↔id; IR333b fija la topología SQL preaplicación y el fallo cerrado de un boolean nullable; IR333c fija wiring M117 y la confirmación E2E. M0M517–M0M520 muerden canonicalización, manifiesto meramente proposed, pérdida de `confirm_operation` y autoridad SQL nula.

## 2. Archivos creados o modificados

- `supabase/sql/117_m0_loop_debt_proceeds_step_authority.sql`: DDL preaplicación.
- `scripts/qa/m0-loop-117-e2e.mjs`: sondas M117.1–M117.3 y cleanup/residuo.
- `src/lib/ai/agent/kipu-agent-tools.ts`: exportación pura del resolver canónico ya usado por el executor; `selectedEntity` acepta catálogos readonly.
- `src/lib/ai/agent/kipu-agent-loop.ts`: canonicalización de identidad objetivo en el dispatcher/consolidación loop.
- `scripts/qa/m0-loop-conversation-e2e.mjs`: `DRY_BORROWED_LINK` post-confirmación con estado exacto.
- `src/app/dev/capture-test/page.tsx`: IR333a–c; retiro de IR332g.
- `scripts/qa/telegram-agent-regression-audit.mjs`: M0M517–M0M520.
- `docs/M0_LOOP_ETAPA_3E_REPORT_2026-08-15.md`: este reporte.

## 3. Decisiones y composición verificadas

### 3.1 Autoridad loop estrictamente durable

La rama loop no toma autoridad de `plan.actions`, del modelo ni de la frase de confirmación. Exige simultáneamente el step corriente del mismo `plan_version`, capability `record_person_payment`, fingerprint server-derived, marcador económico de catálogo y una action del manifiesto que espeja por valor `action_id`, capability, arguments y effects. El consumidor exige `v_intent_authorized is not true`, por lo que `false` y `NULL` rehúsan por igual. `authorize` y `begin` siguen siendo las únicas transiciones que pueden producir el manifiesto `executing` con delivery de autorización distinta.

### 3.2 Rama legacy y cuerpo del writer

El predicado legacy conserva exactamente la búsqueda de `classification='debt_proceeds'` en `v_op.plan.actions`. Un diff mecánico desde `v_base_amount :=` hasta `end;` dio exit 0: cálculo, replay, locks de entidades, ledger, deuda, marker, refs y receipt son idénticos a migración 100. El cambio es sólo la selección de autoridad antes de ese cuerpo.

### 3.3 Marcador económico

El marker requerido es `source='capability_catalog'`, no `receipt`: esta autorización ocurre antes del write. `record_person_payment` pertenece al catálogo económico; si el catálogo dejara de declararlo, la 117 falla cerrada antes de mover caja o deuda.

### 3.4 Canonicalización sin segundo resolver

`canonicalAgentEntityId` delega al mismo `selectedEntity` de exact-id/unique-name que consume el executor. La clave de consolidación recibe el catálogo ya construido en `AgentContext`; source, monto, fecha y confirmación siguen excluidos de la identidad target.

## 4. Salidas íntegras

Orden final: dry-run MOCK preaplicación compatible → tsc → lint → build → capture → mutaciones solas → PostgreSQL actual → checks estáticos. No hubo ningún proceso de QA paralelo a mutaciones.

### 4.1 Dry-run MOCK preaplicación — invocación descartada por sintaxis

La primera invocación pasó varios flags `--scenario=`; el CLI consume una única opción y por eso ejecutó sólo el primero. No fue un defecto de producto ni se contó como corrida final. Salida íntegra:
...
Dry-run MOCK: ejecuta 1 recorridos representativos y valida estáticamente los 50 contratos del catálogo.
...
loopUsage agregado: {"cachedInputTokens":40,"calls":1,"inputTokens":100,"outputTokens":20}
Judge usage agregado: {"cachedInputTokens":0,"calls":1,"inputTokens":180,"outputTokens":45}
Paraphrase usage agregado: {"cachedInputTokens":0,"calls":0,"inputTokens":0,"outputTokens":0}
Costo real acumulado: 0.000604 USD
Calidad promedio: 5.00/5
Costo estimado corrida completa: {"baseline":"native loop","basis":"MOCK token telemetry plus a deterministic paraphrase allowance, scaled to the complete catalog","estimatedTurns":107,"estimatedUsd":0.06,"ratesUsdPerMillion":{"coach":{"cached":0.25,"input":2.5,"output":15},"mini":{"cached":0.1,"input":0.4,"output":1.6}},"scenarios":50,"tokens":{"agent":{"cachedInputTokens":4280,"calls":107,"inputTokens":10700,"outputTokens":2140},"judge":{"cachedInputTokens":0,"calls":50,"inputTokens":9000,"outputTokens":2250},"paraphrase":{"cachedInputTokens":0,"calls":1,"inputTokens":677,"outputTokens":736}}}
M0 tres carriles (loop, MOCK): 1/1 duros verdes
~~~

Exit: 0.

### 4.2 Dry-run MOCK preaplicación — 13 recorridos compatibles

Comando: `node --env-file=.env.local ./scripts/qa/m0-loop-conversation-e2e.mjs --mode=loop --dry-run --scenario=DRY_READ,DRY_WRITE,DRY_SENSITIVE,DRY_ORIGIN,DRY_CAPITAL,DRY_LOAN_OUT,DRY_CORRECTION,DRY_CONSOLIDATION,DRY_POST_WRITE_ABORT,DRY_REPAYMENT,DRY_RENT_AUTHORITY,DRY_LIVE_REPLACEMENT,DRY_OPERATION_SOURCE`

~~~text
Handshake: contract=m0-agent-eval-2026-08-14-subtractive-semantic-plan-m0-11a mode=loop baseline=loop nativo
Catálogo: legacy=24, transcripts=2, aspiracionales=8×3, total=50.
Dry-run MOCK: ejecuta 13 recorridos representativos y valida estáticamente los 50 contratos del catálogo.
...
loopUsage agregado: {"cachedInputTokens":1840,"calls":46,"inputTokens":4600,"outputTokens":920}
Judge usage agregado: {"cachedInputTokens":0,"calls":13,"inputTokens":2340,"outputTokens":585}
Paraphrase usage agregado: {"cachedInputTokens":0,"calls":0,"inputTokens":0,"outputTokens":0}
Costo real acumulado: 0.023032 USD
Calidad promedio: 5.00/5
...
M0 tres carriles (loop, MOCK): 13/13 duros verdes
~~~

Exit: 0. Modelo MOCK; gasto facturado: USD 0. La cifra de costo del runner es valoración sintética de su telemetry MOCK.
...
### 4.5 Build — primer intento bajo sandbox
...
Exit: 1. Causa externa tipada: `SANDBOX_NETWORK/NEXT_FONT_GEIST_UNREACHABLE`; no hubo error TypeScript ni de producto.

### 4.6 Build — reintento con red autorizada
...
✓ Generating static pages using 11 workers (36/36) in 217ms
...
[kipu.route] {"ts":"2026-08-16T00:56:14.223Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":false,"transactionType":"fixed_expense_create"}
[kipu.route] {"ts":"2026-08-16T00:56:14.224Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":true,"transactionType":"fixed_expense_create"}
[kipu.route] {"ts":"2026-08-16T00:56:14.224Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":false,"transactionType":"fixed_expense_create"}
850/850 capture checks
(node:38149) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/capture/capture-matching.ts is not specified and it doesn't parse as CommonJS.
...
Exit: 0. Cardinal neto `848 - IR332g + IR333a + IR333b + IR333c = 850`.

### 4.8 Mutaciones — proceso único de QA
...
M0 mutations: 514/514
~~~

Exit: 0. Todos los mutantes muertos por su detector nombrado; cero crashes y cero anchors distintos de uno.

### 4.9 PostgreSQL vigente (schema 116)

Comando: `node --env-file=.env.local ./scripts/qa/telegram-agent-100-e2e.mjs`

~~~text
(node:41291) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
...
### 4.10 Checks estáticos finales y barrido de writers

Comandos: node checks; `git diff --check`; diff del suffix del writer 100↔117; `rg` de `plan->'actions'`.
...
===== diff check =====
exit 0
===== legacy suffix parity =====
exit 0
===== writer scan =====
exit 0
supabase/sql/100_agent_conversation_integrity.sql:771:  if v_step_count <> jsonb_array_length(coalesce(v_row.plan->'actions','[]'::jsonb)) then
supabase/sql/100_agent_conversation_integrity.sql:979:     or jsonb_typeof(coalesce(v_plan->'actions','[]'::jsonb)) <> 'array' then
supabase/sql/100_agent_conversation_integrity.sql:983:  if jsonb_array_length(coalesce(v_plan->'actions','[]'::jsonb)) > 24
supabase/sql/100_agent_conversation_integrity.sql:1004:           from jsonb_array_elements(coalesce(v_plan->'actions','[]'::jsonb)) action
supabase/sql/100_agent_conversation_integrity.sql:1017:      and jsonb_array_length(coalesce(v_plan->'actions','[]'::jsonb)) = 0
supabase/sql/100_agent_conversation_integrity.sql:1048:  for v_action in select value from jsonb_array_elements(coalesce(v_plan->'actions','[]'::jsonb))
supabase/sql/100_agent_conversation_integrity.sql:1302:  if v_total <> jsonb_array_length(coalesce(v_op.plan->'actions','[]'::jsonb))
supabase/sql/100_agent_conversation_integrity.sql:1554:           from jsonb_array_elements(coalesce(v_op.plan->'actions','[]'::jsonb)) a
supabase/sql/100_agent_conversation_integrity.sql:1638:           from jsonb_array_elements(coalesce(v_op.plan->'actions','[]'::jsonb)) a
supabase/sql/100_agent_conversation_integrity.sql:1687:           from jsonb_array_elements(coalesce(v_op.plan->'actions','[]'::jsonb)) a
supabase/sql/100_agent_conversation_integrity.sql:1742:           from jsonb_array_elements(coalesce(v_op.plan->'actions','[]'::jsonb)) a
supabase/sql/100_agent_conversation_integrity.sql:2108:       select 1 from jsonb_array_elements(coalesce(v_op.plan->'actions','[]'::jsonb)) a,
supabase/sql/117_m0_loop_debt_proceeds_step_authority.sql:95:      select 1 from jsonb_array_elements(coalesce(v_op.plan->'actions','[]'::jsonb)) a,
supabase/sql/112_m0_operation_manifest_authority.sql:265:  v_plan_actions := coalesce(v_op.plan->'actions','[]'::jsonb);
===== protected-table scan =====
exit 1
~~~

Los hits se clasificaron así:

- `100:771`: resume/paridad de plan; no writer.
- `100:979–1048`: save/validación de plan; no writer.
- `100:1302`: verify envelope; no writer.
- `100:1554,1638,1687,1742`: preflight envelope excluido del loop por A2-2.3.
- `100:2108`: `kipu_apply_debt_proceeds`, único writer directo; cubierto por 117.
- `112:265`: registro de manifiesto envelope; no writer.
- `117:95`: rama legacy deliberadamente preservada.

## 5. No ejecutado por disciplina pre-aplicación

- `node --env-file=.env.local ./scripts/qa/m0-loop-117-e2e.mjs`: **NO EJECUTADO — PENDIENTE DE APLICACIÓN DE 117**.
- `DRY_BORROWED_LINK` con confirmación y write exacto: **NO EJECUTADO — PENDIENTE DE APLICACIÓN DE 117**.
- Migración 117: **NO APLICADA**.
- Cero llamadas pagadas, commits, pushes o deploys.

## 6. Riesgos y objeciones

### 6.1 Resultado del barrido definitivo

No refuto la evidencia de A9: `kipu_apply_debt_proceeds` es el único writer alcanzable por loop que exige identidad/effect desde `plan.actions`. Los demás hits son lifecycle, validación, verify, manifest legacy o preflight excluido.

### 6.2 Cobertura funcional deliberadamente diferida

La rama nueva no puede certificarse honestamente contra el schema 116 aplicado. IR333b es sólo smoke de topología preaplicación; la garantía conductual vive en M117.1–M117.3 y `DRY_BORROWED_LINK`, ambos escritos y detenidos hasta que el founder aplique 117.

### 6.3 `md5` frente a `extensions.digest`

La 116 fijó `arguments_fingerprint=md5(arguments::text)` y `record_outcome` exige esa convención. La 117 sólo compara contra el mismo `md5`; no llama pgcrypto. Añadir `extensions.digest` sería una dependencia innecesaria, por lo que el DO-block pinnea `search_path` pero el smoke exige además que la migración no introduzca `extensions.digest`.

### 6.4 Negativo legacy alcanzable por RPC

M117.3 no muta tablas protegidas. Guarda un plan envelope con un effect deliberadamente no-`debt_proceeds` y llama al writer bajo lease; como la autoridad inmutable se comprueba antes de la precondición `preflighted`, el rechazo `unsafe` prueba la rama legacy sin fabricar un estado imposible.

### 6.5 Riesgo residual pre-aplicación

El SQL fue revisado por topología, paridad textual del writer y red preaplicación, pero PostgreSQL aún no lo ha compilado. Cualquier defecto que aparezca al aplicar o ejecutar M117 debe detener la fase post-aplicación y corregirse mediante una migración posterior auditada; no mediante parche al DDL vivo.

## 7. DDL completo de migración 117 — PREPARADO, NO APLICADO
...
La entrega 1W está completa en el árbol local. La migración 117 espera auditoría pre-aplicación y aplicación exclusiva del founder.
