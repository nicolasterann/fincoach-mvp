...
# M0 — Etapa 3D / ronda 1V — Reporte de implementación

Fecha: 2026-08-15  
Estado: implementación terminada; lista para auditoría de Claude.  
Árbol: worktree local no comiteado, sin push, deploy ni cambios al DDL aplicado.  
Baseline de comparación vigente: `on` HÍBRIDO v44+M0.11A; esta ronda sólo modifica el camino `loop`.

## 1. Alcance implementado frente al contrato 1V

1. **Consolidación viva.** Una llamada nueva con la misma capability y el mismo target mecánico reemplaza la versión anterior al reconstruir el manifiesto sucesor. El dispatcher inspecciona todas las llamadas de la delivery antes de restagear: la última versión de cada target gana y conserva la posición del step anterior. Source, monto, fecha y confirmaciones no forman identidad objetivo.
2. **Autoridad de origen en ejecución confirmada.** Sólo cuando `operationManifestAuthorized===true`, una fuente ausente se resuelve primero desde `entityAuthorityMessages` de la operación con el verificador v42 existente. No se sobreescribe una fuente presente, no se acepta candidato único y un nombre de tarjeta que contiene el nombre de una cuenta no autoriza esa cuenta.
3. **FOUR_CREDITS, traza primero.** El manifiesto confirmado de la corrida 1U tenía ocho acciones: cuatro `register_card_payment` (ordinales 5–8) y cuatro `close_card` (9–12). Las cuatro primeras terminaron `done/write`; los cierres quedaron `needs_info`. No estaban fuera del sucesor y no fallaron por una lectura de deuda vieja: entraron al executor sin el boolean legacy `confirm`. La autorización durable del manifiesto ahora satisface ese boolean exclusivamente en el wrapper loop, sin mutar los argumentos persistidos ni reescribir el executor.
4. **Corrección como unidad.** El prompt enseña que, si ya existen valores corregidos, el modelo debe llamar el undo y todos los reemplazos en la misma respuesta de tools. El guion MOCK usa una corrección natural y asierta `undo_agent_operation` antes del lote de reemplazos.
5. **Completitud antes de staging.** Para `record_person_payment(direction=in,inflowKind=borrowed)`, cuenta destino y deuda no-tarjeta se resuelven desde catálogo + autoridad user-authored antes de schema/staging. Si no hay vínculo único, se devuelve una pregunta y no se crea propuesta incompleta.
6. **Detector de conducta.** El detector sólo compara la pregunta terminal normalizada. User delivery idéntica + reply idéntico es neutral; una interrogación embebida en prosa que no termina pidiendo el dato no cuenta; una pregunta terminal igual con snapshot igual sí queda roja.
7. **Red.** IR332a–g, dry-run MOCK 14/14 y mutantes M0M511–M0M516. M0M516 usa un baseline conductual barato propio; el runner de mutaciones sigue exigiendo capture verde y corre cada mutante de forma aislada.
8. **Desviación explícita por imposibilidad preexistente.** La pata `DRY_BORROWED_LINK` certifica sólo completitud y cero writes pre-confirmación. No se falseó un E2E de ejecución: el writer SQL 100 exige `v_op.plan.actions[].effects.classification='debt_proceeds'`, mientras la 116 exige que el plan loop sea exactamente `{"mode":"loop"}`. El bloqueo y su consecuencia para ME12 están detallados en §6.1.

No se usaron frases/regex para capability, entidad, sensibilidad o confirmación. Los únicos procesamientos lingüísticos son el verificador de autoridad v42 ya existente y la normalización de preguntas del harness.

## 2. Archivos creados o modificados

- `src/lib/ai/agent/kipu-agent-loop.ts`: identidad objetivo, reconstrucción multi-call con latest-wins, prompt de corrección unitaria y consumo del pre-staging.
- `src/lib/ai/agent/kipu-agent-tools.ts`: resolución loop-only de fuente durable, completitud de fondos prestados y shim runtime de confirmación de cierre.
- `scripts/qa/m0-loop-conversation-e2e.mjs`: detector compartido, tres escenarios MOCK nuevos, cierre sin `confirm:true` y guion de corrección realista.
- `scripts/qa/m0-loop-conversation-behavior.mjs`: helper puro + IR conductual de replay, interrogación embebida y repetición real.
- `src/app/dev/capture-test/page.tsx`: IR332a–g y actualización de los wiring smokes afectados por el dispatcher.
- `scripts/qa/telegram-agent-regression-audit.mjs`: M0M511–M0M516 y soporte de baseline/comando focal por mutante.
- `docs/M0_LOOP_ETAPA_3D_REPORT_2026-08-15.md`: este reporte.

## 3. Decisiones de diseño

### 3.1 Reemplazo por target y no por intent completo

`agentToolIntentKey` sigue detectando duplicado exacto. Para reemplazo, la identidad omite argumentos mutables y usa campos target publicados por capability. El dispatcher lee toda la respuesta de tools antes de restagear: esto evita que una delivery que actualiza cuatro tarjetas conserve tres versiones viejas o cambie `pays → closes` a `closes → pays`. Si la misma entidad aparece varias veces en una delivery, la última llamada gana.

### 3.2 Confirmación durable como compatibilidad runtime

No se añadió `confirm:true` al manifiesto ni al mock. `loopManifestExecutionArguments` lo aporta sólo al invocar `close_card` desde un manifiesto ya autorizado. Así, el estado durable sigue reflejando exactamente lo que propuso el modelo, mientras el executor legacy recibe la prueba equivalente de segunda delivery. El camino `on` no entra en este branch.

### 3.3 Fuente user-authored antes de S31

La resolución se ejecuta después de montar el shim loop y antes de Saldo/schema/guards. Usa todas las delivery user-authored de la operación, no la frase de confirmación aislada. Una fuente ya presente no se toca; ausencia de una cuenta nombrada de forma inequívoca deja que S31 o el guard normal decidan.

### 3.4 Pregunta terminal, no cualquier signo de interrogación

El carril CONDUCTA no interpreta copy. Extrae únicamente la pregunta con la que efectivamente termina el mensaje. Esto mantiene roja la repetición real, pero no confunde una explicación que cita una pregunta previa con una nueva solicitud de dato.

## 4. Salida íntegra de gates y dry-runs

Orden final: dry-run MOCK completo → tsc → lint → build → capture → mutaciones solas → PostgreSQL. No hubo procesos de QA paralelos a mutaciones.
...
Dry-run MOCK: ejecuta 14 recorridos representativos y valida estáticamente los 50 contratos del catálogo.
...
[DRY_BORROWED_LINK] préstamo recibido resuelve sus vínculos antes de stagear
  DINERO   PASS
  CONDUCTA PASS
  CALIDAD  {"average":5,"correct_numbers":5,"human_coach_voice":5,"model":"MOCK","only_necessary_questions":5,"rationale":"Juez MOCK: transcript enlatado y shape completo validados.","resolved_request":5}
  TRANSCRIPT [{"assistant":"Preparé acreditar 83,86 USD en Produbanco y aumentar la deuda Alpaca. ¿Confirmas esa interpretación?","user":"Alpaca me prestó 83,86 USD y entraron a Produbanco; prepara el préstamo recibido."}]
[DRY_BORROWED_LINK] cleanup por identidad: cero
Residuo de personas por catálogo auth: cero

loopUsage agregado: {"cachedInputTokens":1920,"calls":48,"inputTokens":4800,"outputTokens":960}
Judge usage agregado: {"cachedInputTokens":0,"calls":14,"inputTokens":2520,"outputTokens":630}
Paraphrase usage agregado: {"cachedInputTokens":0,"calls":0,"inputTokens":0,"outputTokens":0}
Costo real acumulado: 0.024096 USD
...
M0 tres carriles (loop, MOCK): 14/14 duros verdes
~~~

Exit: 0.

La cifra “Costo real acumulado” es valoración sintética del telemetry MOCK. Gasto facturado de esta ronda: USD 0.

### 4.2 IR conductual del detector

Comando: `node ./scripts/qa/m0-loop-conversation-behavior.mjs`

~~~text
IR332e · PASS · {"ok":true,"replay":false,"embedded":false,"realRepeat":true,"progressed":false}
~~~
...
Exit: 1. Causa tipada: `SANDBOX_NETWORK/GOOGLE_FONTS_UNREACHABLE`.

### 4.6 Build — reintento autorizado
...
✓ Generating static pages using 11 workers (36/36) in 235ms
...
[kipu.route] {"ts":"2026-08-15T23:31:23.149Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":false,"transactionType":"fixed_expense_create"}
[kipu.route] {"ts":"2026-08-15T23:31:23.149Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":true,"transactionType":"fixed_expense_create"}
[kipu.route] {"ts":"2026-08-15T23:31:23.149Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":false,"transactionType":"fixed_expense_create"}
848/848 capture checks
(node:25452) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/capture/capture-matching.ts is not specified and it doesn't parse as CommonJS.
...
### 4.8 Mutaciones — ejecutadas solas
...
M0 mutations: 510/510
~~~

Exit: 0.

### 4.9 PostgreSQL — intento bajo sandbox

Comando: `node --env-file=.env.local ./scripts/qa/telegram-agent-100-e2e.mjs`

~~~text
(node:29038) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
...
Exit: 1. Causa tipada: `SANDBOX_NETWORK/SUPABASE_DNS_UNREACHABLE`.

### 4.10 PostgreSQL — reintento autorizado

Comando: `node --env-file=.env.local ./scripts/qa/telegram-agent-100-e2e.mjs`

~~~text
(node:29048) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use node --trace-warnings ... to show where the warning was created)
...
- No se ejecutó la corrida final 2 real. Queda pre-autorizada sólo después del veredicto de Claude sobre 1V.
- No se llamó ningún modelo real ni se generó gasto facturado; el dry-run usó completions y juez MOCK.
- No se cambió ni aplicó DDL, no se ejecutaron migraciones y no se alteró el schema vivo.
- No se reescribieron executors. La compatibilidad de `close_card` vive en el wrapper loop.
- No se cambió el camino `KIPU_AGENT_MODE=on`; todo consumo nuevo en `executeTool` está detrás de `loopMode`.
- No se modificaron carriles, juez, umbrales ni escenarios reales para acomodar resultados.
- No se certificó la ejecución loop de fondos prestados por la incompatibilidad §6.1.

## 6. Riesgos y objeciones

### 6.1 [P1] PRODUCT_SQL/DEBT_PROCEEDS_LEGACY_PLAN_SHAPE_INCOMPATIBLE_WITH_M116

La completitud pedida por A8 ya funciona: el manifiesto MOCK stageó por valor:

~~~json
{"accountId":"<Produbanco-id>","amount":83.86,"debtAccountId":"<Alpaca-id>","direction":"in","inflowKind":"borrowed","occurredAtISO":"2026-08-15","person":"Alpaca"}
~~~

Sin embargo, confirmar esa propuesta llega al RPC `kipu_apply_debt_proceeds` de la 100. En `100_agent_conversation_integrity.sql:2108–2111`, la identidad inmutable exige que el mismo step aparezca en `v_op.plan.actions` con un effect cuya `classification` sea `debt_proceeds`. La 116 fija y verifica el plan loop como exactamente `{"mode":"loop"}` en 116:62, 182, 249, 401, 510 y 626. Esas dos precondiciones no pueden ser verdaderas a la vez.

Esto explica ME12 rojo en todas las corridas loop y la traza 1U: la propuesta era correcta, pero la confirmación devolvía `record_person_payment needs_info` sin escribir. El PostgreSQL 82/82 verde no refuta la incompatibilidad: M100.9 prueba el writer con un plan legacy que sí contiene `actions/effects`, no con una operación M116.

No existe corrección honesta dentro de “cero DDL”: `agent_operations` está protegido contra mutación directa, `save_plan` está excluido del loop y cambiar el executor para escribir caja/deuda por separado rompería atomicidad. La corrección necesita una migración 117 auditada (por ejemplo, hacer que el writer acepte identidad/effect desde el step loop stageado bajo las mismas pruebas de lease/ownership/manifest), o una decisión explícita equivalente de arquitectura. Por eso el dry-run de 1V asierta sólo staging completo y mantiene un smoke IR332g que impide presentar esa pata como E2E ejecutada.

### 6.2 [P2] Precisión sobre la bifurcación de traza FOUR_CREDITS de A8

A8 proponía decidir entre “cierres fuera del sucesor” y “cierres ejecutados/refusados por freshness de deuda”. La evidencia durable muestra una tercera causa concreta: los cierres estaban dentro del manifiesto, se invocaron, y devolvieron `needs_info` porque `close_card` todavía esperaba `args.confirm===true`. El refresh ya corría entre writes. La solución implementada usa la autorización del manifiesto como ese segundo delivery, sin relajar el chequeo de saldo/deuda del executor.

### 6.3 Confianza residual de identidad objetivo

La clave target es mecánica y no usa texto del usuario. Dentro de `register_card_payment`, `cardName` admite nombre o id bajo la misma propiedad. Dos deliveries que representen la misma tarjeta una vez por nombre y otra por UUID producirían claves diferentes antes de consultar catálogo. Los escenarios observados y schemas reales conservan la misma representación entre propuesta y modificación; si Claude exige paridad nombre↔id como parte de “misma entidad”, conviene canonicalizar ese campo contra el catálogo antes de comparar. No se amplió unilateralmente el contrato a un router genérico.

## 7. DDL propuesto

Ninguno. No se creó ni aplicó una migración 117. §6.1 documenta por qué será necesaria si se exige cerrar ME12 dentro de la vara A7-3.
