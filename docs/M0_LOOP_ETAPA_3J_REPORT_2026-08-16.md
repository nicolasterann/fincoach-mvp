# M0 loop — Reporte Etapa 3J / contrato 1AB

Fecha: 2026-08-16  
Estado: implementación y gates completos; lista para auditoría de Claude  
Llamadas pagadas: cero

## 1. Alcance implementado frente al contrato 1AB

1. **Observabilidad del catch exterior y post-write:** LoopDiagnostic incorpora turnFailure con site cerrado (round_completion, forced_completion, finalize, dispatch, outer) y token acotado. Se conserva sólo un KIPU_* válido, o nombre de constructor más status HTTP 100–599, o unknown_error.
2. **Durabilidad:** el diagnóstico compuesto cruza el resultado del loop, el handler ya existente lo persiste en metadata del mensaje asistente y turnDetail del E2E imprime el objeto completo antes del cleanup. Ningún mensaje crudo del error ni texto del usuario entra al diagnóstico.
3. **Settle antes de continuidad:** todos los catches de narración con receipt útil intentan settleDurableWork antes de publicar continuidad. El catch exterior con wrote=true hace el mismo intento. Si settle falla, el diagnóstico 1AA queda asentado y la continuidad veraz se conserva.
4. **Reproducción gratis del bug latente:** DRY_SUCCESSOR_PAY_CLOSE_READ añadió list_open_receivables después de confirm_operation y antes de narrar. Antes del fix reprodujo el rojo: las ocho acciones se ejecutaron, el manifiesto quedó executing y la lectura cayó en el rechazo de staging; exit 1.
5. **Fix mínimo probado:** el redirect de un manifiesto executing alcanza ahora también las lecturas. La lectura recibe el control result manifest_executing_read_deferred y el modelo narra los receipts ya ejecutados; no se stagea ningún step nuevo. No fue necesaria migración 118.
6. **Dirección inversa y fronteras:** DRY_WRITE, cohesión 1Y, redirects 1Z, 117/1X, writers SQL, envelope/on y todos los demás reads fuera de executing permanecen verdes e intactos. No hay routing por texto del usuario.
7. **Resultado:** focal post-fix 1/1; batería completa MOCK 20/20, residuo cero.

Desviaciones: ninguna del contrato. La mecánica elegida fue la segunda opción explícita de A21 (extender el redirect a lecturas), por ser la única solución TS-local que evita staging bajo un manifiesto executing sin relajar la barrera SQL.

## 2. Archivos creados o modificados en 1AB

- src/lib/ai/agent/kipu-agent-loop.ts — turnFailure acotado, settle-before-continuity y redirect de lecturas durante executing.
- scripts/qa/m0-loop-conversation-e2e.mjs — DRY_SUCCESSOR_PAY_CLOSE_READ y aserción durable de continuidad post-write.
- src/app/dev/capture-test/page.tsx — IR338 conductual y wiring de persistencia, settle y redirect.
- scripts/qa/telegram-agent-regression-audit.mjs — M0M527–M0M529.
- scripts/qa/README.md — cardinal de 20 patas y documentación de la variante post-exec.
- docs/M0_LOOP_ETAPA_3J_REPORT_2026-08-16.md — esta acta.

## 3. Decisiones de diseño y evidencia traza-primero

### 3.1 Gramática cerrada de turnFailure

La extracción busca únicamente KIPU_[A-Z_]{2,80}. Sin ese token, admite un constructor alfanumérico server-bounded de hasta 64 caracteres y, si existe, un status HTTP entero 100–599 como sufijo _HTTP_n. Cualquier otro thrown value cae en unknown_error. IR338 prueba KIPU_VALIDATION, IR338TransportError_HTTP_429 y un string opaco que no sobrevive.

### 3.2 Settle idempotente antes de continuidad

El predicado puro loopShouldSettleBeforeContinuity exige simultáneamente write, claim durable, no-settled y no-attempt previo. El helper llama settleDurableWork una sola vez. Los cuatro catches de narración y el catch exterior lo invocan antes de componer la continuidad. DRY_POST_WRITE_ABORT demuestra write exacto, step verified, operación completed, continuidad con 25 y turnFailure forced_completion/Error sin settleFailure.

### 3.3 Elección del redirect para lecturas post-ejecución

Ejecutar la lectura sin staging habría creado una excepción lateral dentro del dispatcher; relajar SQL habría requerido DDL y ampliado una barrera correcta. El redirect ya era la identidad determinista del estado executing. Extenderlo a reads conserva un solo mecanismo: no ejecuta, no stagea, no duplica y devuelve al modelo la instrucción server-owned de narrar primero los receipts confirmados.

Reproducción pre-fix, evidencia estructurada preservada:

~~~text
[DRY_SUCCESSOR_PAY_CLOSE_READ] lectura post-ejecución no bloquea el settle del sucesor
  DINERO   FAIL · post-execution read cannot strand the executing manifest
  CONDUCTA FAIL
  DIAGNÓSTICO: ocho writes exactos; manifest.status="executing"; verification=null; loopDiagnostic={"code":"validation","stage":"turn"}
[DRY_SUCCESSOR_PAY_CLOSE_READ] cleanup por identidad: cero
M0 tres carriles (loop, MOCK): 0/1 duros verdes
Exit: 1
~~~

Post-fix focal: DINERO PASS, CONDUCTA PASS, cleanup cero, 1/1, exit 0.

### 3.4 Red y cardinales

- Capture previo: 854. IR338 agrega una aserción: 854 + 1 = **855**.
- Mutaciones previas: 520. M0M527–M0M529: 520 + 3 = **523**.
- Dry previo: 19. DRY_SUCCESSOR_PAY_CLOSE_READ: 19 + 1 = **20**.
- M0M527 elimina el status HTTP del token; M0M528 desactiva conductualmente el predicado de settle; M0M529 restaura la exención de reads. Los tres mueren por IR338.
- El primer intento de M0M528 sobrevivió porque el smoke sólo fijaba la presencia de la llamada. Se reemplazó por el predicado puro usado en producto y sus dos sentidos; el mutante final es conductual y murió por IR338.

### 3.5 Incidentes de harness durante cierre

1. **HARNESS_SCOPE/DURABLE_STEPS_OUT_OF_SCOPE:** la primera corrida completa localizó durableSteps por error en DRY_LOAN_OUT; DRY_POST_WRITE_ABORT abortó con ReferenceError. Se movió la misma lectura al fixture correcto; producto intacto.
2. **HARNESS_EXPECTATION/TURN_FAILURE_SITE:** la segunda corrida probó verified/completed y continuidad, pero el assert esperaba finalize. La observabilidad nueva nombró correctamente forced_completion; se alineó el fixture con la sub-etapa real.
3. La tercera corrida, sin más cambios, cerró 20/20 y residuo cero.
...
~~~text
(node:21906) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
...
### 4.2 Dry-run MOCK loop — 20 patas
...
Dry-run MOCK: ejecuta 20 recorridos representativos y valida estáticamente los 50 contratos del catálogo.
...
loopUsage agregado: {"cachedInputTokens":3720,"calls":93,"inputTokens":9300,"outputTokens":1860}
Judge usage agregado: {"cachedInputTokens":0,"calls":20,"inputTokens":3600,"outputTokens":900}
Paraphrase usage agregado: {"cachedInputTokens":0,"calls":0,"inputTokens":0,"outputTokens":0}
Costo real acumulado: 0.045660 USD
Calidad promedio: 5.00/5
Costo estimado corrida completa: {"baseline":"native loop","basis":"MOCK token telemetry plus a deterministic paraphrase allowance, scaled to the complete catalog","estimatedTurns":108,"estimatedUsd":0.12,"ratesUsdPerMillion":{"coach":{"cached":0.25,"input":2.5,"output":15},"mini":{"cached":0.1,"input":0.4,"output":1.6}},"scenarios":50,"tokens":{"agent":{"cachedInputTokens":9343,"calls":234,"inputTokens":23358,"outputTokens":4672},"judge":{"cachedInputTokens":0,"calls":50,"inputTokens":9000,"outputTokens":2250},"paraphrase":{"cachedInputTokens":0,"calls":1,"inputTokens":677,"outputTokens":736}}}
M0 tres carriles (loop, MOCK): 20/20 duros verdes
~~~

Exit: 0. Resultado: 20/20; residuo de personas cero. El costo impreso es valoración sintética de telemetría MOCK; no hubo consumo facturado.
...
[kipu.route] {"ts":"2026-08-16T18:11:29.122Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":false,"transactionType":"fixed_expense_create"}
[kipu.route] {"ts":"2026-08-16T18:11:29.123Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":true,"transactionType":"fixed_expense_create"}
[kipu.route] {"ts":"2026-08-16T18:11:29.123Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":false,"transactionType":"fixed_expense_create"}
855/855 capture checks
(node:22340) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/capture/capture-matching.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
~~~

Exit: 0. Resultado: 855/855.
...
✓ Generating static pages using 11 workers (36/36) in 210ms
...
M0 mutations: 523/523
~~~

Exit: 0. Resultado: 523/523; la auditoría corrió sola, los tres mutantes 1AB murieron por IR338, cero crashes y cero anchors distintos de uno.
...
~~~text
(node:25636) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
...
- No hubo llamadas de modelo pagadas; todo el modelo fue MOCK.
- No se modificó ni aplicó DDL. No existe migración 118.
- No se tocaron SQL, writers, cuerpos de executors, 117/1X, cohesión 1Y, redirects 1Z ni el camino envelope/on.
- No se ejecutó la siguiente enfocada real: espera autorización de Claude.
- No hubo commit, push ni deploy.

## 6. Riesgos y objeciones

### 6.1 La causa real de la enfocada aún debe volver a nombrarse

1AB garantiza que el próximo catch exterior deja site+token y que todo write intenta settle antes de continuidad. No afirma retrospectivamente cuál fue el throw real de ME17/REAL_FOUR_CREDITS. Esa evidencia requiere la enfocada autorizada posterior.

### 6.2 Trade-off deliberado del redirect de reads

Durante un manifiesto executing, una lectura emitida por el modelo se difiere y no obtiene datos frescos dentro del mismo turno. La respuesta conserva receipts reales y el manifiesto termina verified; una lectura ulterior normal sigue disponible. Considero este trade-off menor que abrir una segunda semántica de steps o relajar la barrera SQL.

### 6.3 Constructor de error como clase, no detalle

Un proveedor que lance siempre Error sin status producirá token Error. Es menos específico que un KIPU_* pero cumple la gramática cerrada y evita persistir texto libre. IR338 fija el status cuando existe.

## 7. DDL propuesto

Ninguno. 1AB es TS-only; no se aplicó ni redactó migración.
