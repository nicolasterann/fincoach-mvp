# M0 — Etapa 3H / ronda 1Z — Redirect durable de re-emisiones económicas

Fecha: 2026-08-16  
Árbol: local, sin commit  
Contrato: ADENDA 17 / 1Z  
Resultado: implementación completa; gates verdes; cero llamadas pagadas.

## 1. Alcance implementado frente al contrato 1Z

Se implementó la cohesión de autorización observable para una propuesta durable ya existente:

- una completion se normaliza y compara como conjunto completo antes de despachar su primera mutación;
- si las capabilities y argumentos normalizados son value-identical al manifiesto `proposed`, el dispatcher devuelve `status: "redirect"`, `effect: "noop"` y control tipado `pending_manifest_value_identical`; no ejecuta, no stagea, no crea sucesor y no vuelve a proponer;
- el resultado indica el `manifestId` y `operationId` durables y ordena al modelo usar `confirm_operation` o `reject_operation`;
- un conjunto modificado conserva sin cambios el flujo existente reject → bump → re-staging → manifiesto sucesor;
- después de una confirmación aceptada, toda nueva mutación emitida mientras el manifiesto está `executing` recibe `manifest_already_executing`, sin parsear argumentos ni entrar a writers;
- una propuesta retenida de una delivery anterior ya no cuenta como propuesta creada por la delivery actual para el guard anti-self: sólo `stagedSensitive.length` mide autoría actual;
- una confirmación exitosa marca el manifiesto pendiente como manejado y limpia el estado request-local de retención, de modo que el settle verifica steps y manifiesto en vez de dejarlos en `executing`;
- el viejo fallback por acción duplicada deja de fijar `outcome.needsInfo` prematuramente; si no llega confirm/reject, la transición terminal lo fija desde `retainedProposedManifest`, y si sí llega confirmación no queda un waiver stale.

La frontera usa exclusivamente la identidad mecánica existente `agentToolIntentKey(capability, arguments)`, con igualdad de multiconjunto independiente del orden. No interpreta el mensaje del usuario.

No hubo desvíos del contrato. No se modificaron DDL, migraciones 117/1X, writers SQL, cuerpos de executors ni el camino envelope/on.

## 2. Archivos creados o modificados en 1Z

- `src/lib/ai/agent/kipu-agent-loop.ts` — comparador de conjunto durable, clasificación pre-dispatch, redirects tipados pending/executing y composición correcta con confirmación/anti-self.
- `scripts/qa/m0-loop-conversation-e2e.mjs` — tres patas MOCK de re-emisión, reparación lifecycle de ME5/ME16 y aserciones PostgreSQL de identidad, sucesión, paridad y no duplicación.
- `src/app/dev/capture-test/page.tsx` — IR336 conductual y wiring de las dos direcciones más no-crash.
- `scripts/qa/telegram-agent-regression-audit.mjs` — M0M524, M0M525 y M0M526.
- `docs/M0_LOOP_ETAPA_3H_REPORT_2026-08-16.md` — esta acta 1Z.

## 3. Decisiones de diseño y evidencia traza-primero

### 3.1 Identidad de conjunto y orden

`loopPendingManifestSetDisposition` convierte cada acción durable y cada call normalizada en la clave ya probada `agentToolIntentKey`, ordena ambas listas y exige igualdad de longitud y valor. Por eso una permutación del mismo lote es idéntica, la multiplicidad se conserva y cualquier capability/argumento nuevo resulta `modified`.

Antes del `for` de dispatch, el loop normaliza todas las mutaciones no-control/no-read de la completion. Sólo un lote completo, válido e idéntico obtiene redirect. Un lote modificado no recibe autoridad ni ejecución: sigue exactamente la consolidación ya existente.

### 3.2 No-crash durante executing

El redirect `manifest_already_executing` se decide antes de `safeArgs`, completitud, schema, staging o writer. Así, incluso una re-emisión inválida después de `confirm_operation` no puede convertir receipts ya escritos en `unavailable`; el modelo sólo puede narrar los receipts de la confirmación.

### 3.3 Causa raíz de final-3 y ME5

La traza preservada de `final-3-1y` mostró que ME17 y REAL_FOUR_CREDITS ejecutaban las ocho acciones del manifiesto y luego una completion adicional volvía a emitirlas. La delivery había reclamado una propuesta previa, pero `retainedProposedManifest` se sumaba a `stagedActionCount`: el guard la clasificaba falsamente como auto-confirmación de la misma delivery. Además el estado `needsInfo` se fijaba antes de saber si el modelo llamaría después a `confirm_operation`. Clase: `PRODUCT_LOOP/PENDING_REEMISSION_MISCLASSIFIED_AS_SELF_DECISION`.

ME5 no era la misma falla de consumo. Su ficha titulaba “inspecciona sin consumir”, pero después de 1Y todavía exigía tres pagos inmediatos y sólo difería la cuarta acción. Eso contradice por construcción la cohesión del conjunto. Clase: `HARNESS_LIFECYCLE/ME5_PRE_1Y_PARTIAL_WRITE_EXPECTATION`. La reparación exige cero writes, una sola propuesta de cuatro acciones y el mismo `id`, `operation_id` y `manifest_hash` antes/después de “¿Qué dato te falta?”.

ME16 sólo tenía dos turnos: lectura y solicitud de cuatro pagos. Nunca enviaba la delivery de confirmación requerida. Se agregó un tercer turno natural y la vara ahora prueba una propuesta de cuatro acciones que termina `verified` con el mismo hash.

### 3.4 Red y cardinales

- Capture previo: 852. IR336 agrega una aserción: 852 + 1 = 853.
- Mutaciones previas: 517. M0M524 + M0M525 + M0M526: 517 + 3 = 520.
- Dry previo: 15 patas. `DRY_CONFIRM_REEMIT_IDENTICAL`, `DRY_CONFIRM_REEMIT_MODIFIED` y `DRY_EXECUTING_REEMIT`: 15 + 3 = 18.
- IR336 prueba igualdad aun con orden permutado, desigualdad por valor, clasificación antes del dispatch, ambos redirects, anti-self limitado a autoría actual, DRY_WRITE y DRY_SET_COHESION intactos.
- M0M524 confunde `modified` con `identical`; M0M525 reintroduce el falso self-decision; M0M526 permite que una re-emisión salte el redirect de `executing`. Los tres mueren por IR336.

## 4. Salidas íntegras de gates

Todos los gates se ejecutaron en orden serial. La auditoría de mutaciones corrió sola, con el dev server detenido y sin otro gate en paralelo. Los exit codes se leyeron directamente, sin pipes.

### 4.1 M117 post-aplicación

Comando: `node --env-file=.env.local ./scripts/qa/m0-loop-117-e2e.mjs`

~~~text
(node:95269) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
...
Exit: 0. Resultado: 3/3; cleanup del harness completado.

### 4.2 Dry-run MOCK loop — 18 patas

Comando: `node --env-file=.env.local ./scripts/qa/m0-loop-conversation-e2e.mjs --mode=loop --dry-run`, con servidor local `KIPU_AGENT_MODE=loop npm run dev`.
...
Dry-run MOCK: ejecuta 18 recorridos representativos y valida estáticamente los 50 contratos del catálogo.
...
loopUsage agregado: {"cachedInputTokens":3120,"calls":78,"inputTokens":7800,"outputTokens":1560}
Judge usage agregado: {"cachedInputTokens":0,"calls":18,"inputTokens":3240,"outputTokens":810}
Paraphrase usage agregado: {"cachedInputTokens":0,"calls":0,"inputTokens":0,"outputTokens":0}
Costo real acumulado: 0.038472 USD
Calidad promedio: 5.00/5
Costo estimado corrida completa: {"baseline":"native loop","basis":"MOCK token telemetry plus a deterministic paraphrase allowance, scaled to the complete catalog","estimatedTurns":108,"estimatedUsd":0.12,"ratesUsdPerMillion":{"coach":{"cached":0.25,"input":2.5,"output":15},"mini":{"cached":0.1,"input":0.4,"output":1.6}},"scenarios":50,"tokens":{"agent":{"cachedInputTokens":9627,"calls":241,"inputTokens":24069,"outputTokens":4814},"judge":{"cachedInputTokens":0,"calls":50,"inputTokens":9000,"outputTokens":2250},"paraphrase":{"cachedInputTokens":0,"calls":1,"inputTokens":677,"outputTokens":736}}}
M0 tres carriles (loop, MOCK): 18/18 duros verdes
~~~

Exit: 0. Resultado: 18/18; residuo de personas cero. El costo impreso es valoración sintética de telemetría MOCK; no hubo consumo facturado.

### 4.3 TypeScript

Comando: `npm exec tsc -- --noEmit`
...
[kipu.route] {"ts":"2026-08-16T13:45:09.693Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":false,"transactionType":"fixed_expense_create"}
[kipu.route] {"ts":"2026-08-16T13:45:09.693Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":true,"transactionType":"fixed_expense_create"}
[kipu.route] {"ts":"2026-08-16T13:45:09.693Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":false,"transactionType":"fixed_expense_create"}
853/853 capture checks
(node:95536) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/capture/capture-matching.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
~~~

Exit: 0. Resultado: 853/853.

### 4.6 Build

Primer intento dentro del sandbox. Comando: `npm run build`.
...
Exit: 1. Causa: `ENVIRONMENT_SANDBOX/NEXT_FONT_NETWORK_DENIED`; no fue un fallo del árbol. Se repitió una vez fuera del sandbox, sin cambiar archivos.

Segundo intento autorizado fuera del sandbox. Comando idéntico: `npm run build`.
...
ok · M0M9 single-operation undo bypasses the versioned reversal dispatcher → IR95
...
ok · M0M15 planner read evidence is derived but no longer consumed → TG-8
...
ok · M0M207 production derives the local date but never gives it to the planner → IR113c
...
M0 mutations: 520/520
~~~

Exit: 0. Resultado: 520/520; M0M524–526 murieron por IR336; cero crashes y cero anchors distintos de uno.

### 4.8 PostgreSQL — salida íntegra

Comando: `node --env-file=.env.local ./scripts/qa/telegram-agent-100-e2e.mjs`

~~~text
(node:99728) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
...
Exit: 0. Resultado: 82/82.

## 5. Qué no se hizo y qué queda pendiente

- No se hicieron llamadas pagadas. El dry-run usó exclusivamente el modelo MOCK.
- No se ejecutó reproducción focal real ni corrida completa; ambas esperan el OK de Claude previsto por ADENDA 17.
- No se creó, cambió ni aplicó DDL.
- No se modificaron migraciones 116/117 ni el schema vivo.
- No se modificaron writers SQL ni cuerpos lógicos de executors.
- No se modificó el camino `KIPU_AGENT_MODE=on` ni se desplegó producción.
- No se hizo commit, push ni deploy.
- No se escribió contra la cuenta real del founder; todas las personas/filas fueron fixtures marcados y limpiados por identidad.

## 6. Riesgos y objeciones

### 6.1 Identidad de conjunto por completion

La igualdad estricta se aplica al conjunto de mutaciones de una completion. Si el modelo emite sólo un subconjunto de un manifiesto pendiente, ese lote no es value-identical y sigue la consolidación existente como conjunto modificado. Esto es coherente con 1Z: un subconjunto no tiene autoridad para ejecutar y no puede fingir que confirmó el total. La red cubre el conjunto completo con orden permutado y el camino modificado.

### 6.2 Read-only junto a re-emisión

Las tools read-only no participan de la identidad económica del conjunto y se ejecutan normalmente. La igualdad se calcula sólo sobre mutaciones no-control; por tanto una lectura adicional no crea un sucesor económico. Esto preserva la libertad del modelo para inspeccionar antes de confirmar sin convertir una lectura en autoridad.

### 6.3 Incidente focal de QA

El primer focal de las patas idéntica/executing cayó con `HARNESS_ADVISORY/MOCK_EXHAUSTED_ON_DERIVED_TOTAL`: el guion narraba 23,33 USD, suma correcta pero no cifra literal de los receipts 11,11 y 12,22; el advisory pidió una cuarta completion no enlatada. Una línea temporal de `console.error` reveló `mock exhausted at call 4`, se revirtió antes del fix y el archivo quedó sin instrumentación. Se corrigió sólo el guion para citar ambos receipts; las patas pasaron 2/2 y luego 18/18. No fue un fallo del redirect ni un cambio de producto.

### 6.4 Build en sandbox

El primer build no pudo descargar Geist por la restricción de red del sandbox. Sin modificar el árbol se repitió una vez con acceso de red autorizado y pasó. Se conservan ambas salidas íntegras arriba.

No hay objeción de arquitectura ni bloqueo pendiente.

## 7. DDL propuesto

Ninguno. 1Z es TS/harness/IR y no requiere migración.
