...
# M0 Tool Loop — Etapa 1D — reporte pre-aplicación
...
Se leyó completo `docs/M0_CLAUDE_AUDIT_TOOL_LOOP_2026-08-14.md`, incluida la ADENDA 3 A3-1–A3-4. La entrega corrige la implementación 1C sin aplicar DDL y mantiene el modo nativo exclusivamente detrás de `KIPU_AGENT_MODE=loop`.

Verificaciones previas y condición de parada:

1. `kipu_reverse_agent_operation(jsonb)` fue releída antes del DDL: no consume `plan.actions` para ninguna decisión. Resuelve el step durable, clasifica dinero mediante el helper compartido, exige receipts y sólo acota `transaction_ids` a 1..360. La condición de parada A3 quedó satisfecha.
2. Se verificó en el catálogo aplicado que pgcrypto vive en `extensions`; los dos usos de `digest` de 116-r2 están calificados.
3. Se verificó la composición real de claim/replay, `record_outcome`, authorize de 112 y begin de 115. Claim de recuperación devuelve la operación a `planning`; por eso el resume determinista hace `planning → ready → begin application`, reentra el manifiesto `executing`, ejecuta únicamente filas persistidas restantes y asienta antes de narrar.
4. La rama `on` sigue llamando `runKipuAgent`; el helper de evidencia selecciona `runKipuAgentLoop` sólo cuando el modo es literalmente `loop`. No se reescribieron cuerpos de executors.

Alcance entregado:

1. 116-r2 con `extensions.digest`, replay de staging por `capability + arguments_fingerprint` dentro de la misma versión, marcador económico post-exec derivado de transaction receipts poseídos, helper de undo con sources `capability_catalog|receipt` y contención sin filtro por source.
2. La paridad del manifiesto conserva el espejo de staging y admite exclusivamente la adición server-owned del único marcador `receipt`; ninguna propiedad, argumento o efecto económico model-authored se relaja.
3. Settle con lectura fresca de la operación y verificación de **todos** los steps `applied`, incluidos los ordinarios de un turno mixto anterior.
4. Resume determinista de manifiesto `executing`: reentrada sin modelo, ejecución idempotente de los restantes, settle step→manifest y sólo después narración `tool_choice=none`.
5. Reject-only termina `awaiting_input` con `pendingQuestion=finalText`; reject→re-staging en la misma delivery conserva el flujo de A2.
6. `loopDispatcherAuthorized` dejó de ser authority. El guard devuelve `serverAuthorized:false` más la pregunta natural y un marcador tipado; el dispatcher mantiene el step `preflighted` y lo incorpora a la propuesta. Esto cubre sensibilidad mecánica, evidencia monetaria, entidad no probada, duplicado y cuenta habitual con un único manifiesto.
7. `correct_movement` se añadió al set mecánico de segunda delivery (32 capabilities + 8 reglas); el cardinal se actualizó en el documento maestro y quedó fijado por prueba.
8. Evidence capture acepta `on|loop` y enruta el runner correcto tanto en captura directa como en resume de estado de cuenta.
9. Fallos de authorize/reject se entregan al modelo como `tool_result` tipado y permiten continuar/re-proponer; register produce un `tool_result` sintético acotado para explicación segura. El catch exterior sólo conserva `{stage,code}` sin prompts, mensajes ni payloads.
10. QA conductual: advisory con una única reparación mock, anti-fuga por guard puro, auto-decisión TS, guard de entidad con `serverAuthorized:false`; smokes de wiring identificados explícitamente como tales.
11. Sondas M116.1–M116.6 endurecidas: delivery ownership, espejo por valor, anti-self reject+authorize, marcador contextual por receipt, error específico de contención, undo parcial fail-closed y valoración de montos/balance.
12. Dry-run endurecido: manifest actions exactas, receipts verificados, auto-confirmación rehusada y cleanup que no enmascara el error primario.
13. Cuatro mutantes nuevos muerden staging mecánico/monetario, entidad no probada, consumo del marcador `receipt` y settle sobre todos los steps.

Desviaciones del contrato: **ninguna**. La migración y las pruebas que requieren 116 aplicada están escritas pero no ejecutadas.

## 2. Archivos creados/modificados

- `supabase/sql/116_m0_native_agent_loop.sql`: 116-r2 completa; no aplicada.
- `src/lib/ai/agent/kipu-agent-loop.ts`: fresh settle, resume determinista, staging de challenges, fallos tipados y helpers conductuales de salida/anti-self.
- `src/lib/ai/agent/agent-action-guard.ts`: el dispatcher loop deja de autorizar y devuelve el requisito tipado para staging.
- `src/lib/ai/agent/agent-operation-authority.ts`: `correct_movement` entra al catálogo mecánico; cardinal 32.
- `src/lib/capture/evidence-capture.ts`: routing explícito `on|loop` en captura y reanudación.
- `src/lib/ai/chat-transaction-handler.ts`: metadata acotada `loopDiagnostic`.
- `src/app/dev/capture-test/page.tsx`: IR328a–IR328j, con (g)/(h) conductuales y smokes nombrados.
- `scripts/qa/m0-loop-116-e2e.mjs`: sondas M116.1–M116.6 reforzadas; no ejecutadas.
- `scripts/qa/m0-loop-mock-dry-run.mjs`: dry-run MOCK reforzado; no ejecutado.
- `scripts/qa/telegram-agent-regression-audit.mjs`: M0M497–M0M500.
- `docs/M0_CLAUDE_AUDIT_TOOL_LOOP_2026-08-14.md`: cardinal vivo 32 + 8.
- `docs/M0_LOOP_ETAPA_1D_REPORT_2026-08-14.md`: este reporte.

## 3. Decisiones de diseño

1. **Receipt marker y espejo.** Register conserva el valor stageado. Verify puede agregar sólo `economic_event/receipt` después de validar ownership; verify-manifest acepta exactamente `effects_stageados` o `effects_stageados + ese marcador`. Así el atestado post-exec no falsea la propuesta y tampoco rompe su paridad.
2. **Fresh settle como única autoridad runtime.** Los arrays locales se conservan sólo para telemetría inmediata; la promoción durable itera exclusivamente la foto fresca de `operation.steps`.
3. **Recuperación antes de narración.** Una operación recuperada con manifiesto `executing` no entra al loop del modelo. El modelo recibe sólo receipts ya verificados y no dispone de tools en esa narración.
4. **Challenge tipado, no bypass.** El guard no infiere intención nueva: reutiliza sus disparadores existentes y marca el resultado para que el dispatcher stagee la llamada exacta ya persistida.
5. **Diagnóstico acotado.** Las causas SQL se reducen a `superseded|conflict|ownership|validation|unavailable`; nunca se persiste el error crudo. Authorize/reject siguen dentro del loop de tools y por tanto el modelo puede preparar una alternativa en la misma delivery.
6. **Pruebas pre/post aplicación separadas.** IR328g/h/f prueban conducta sin 116. Las garantías de SQL, ciclo completo y receipts permanecen en M116/dry-run y no se simulan.

## 4. Salida íntegra de los gates

Todos los gates finales se ejecutaron en serie. La auditoría de mutaciones corrió sola; PostgreSQL empezó únicamente después de que terminó 494/494.
...
Comando final: `npm run build` (red autorizada únicamente para Google Fonts)
...
[kipu.route] {"ts":"2026-08-14T22:35:48.851Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":false,"transactionType":"fixed_expense_create"}
[kipu.route] {"ts":"2026-08-14T22:35:48.851Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":true,"transactionType":"fixed_expense_create"}
[kipu.route] {"ts":"2026-08-14T22:35:48.851Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":false,"transactionType":"fixed_expense_create"}
816/816 capture checks
(node:10465) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/capture/capture-matching.ts is not specified and it doesn't parse as CommonJS.
...
### 4.5 Mutaciones M0 — ejecutadas solas
...
M0 mutations: 494/494
~~~

Exit: 0

### 4.6 PostgreSQL actual — intento bajo sandbox

Comando: `node --env-file=.env.local ./scripts/qa/telegram-agent-100-e2e.mjs`

~~~text
(node:13647) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
...
El proceso falló exclusivamente por DNS antes de crear la persona desechable; se repitió con red autorizada.

### 4.7 PostgreSQL actual — red autorizada

Comando: `node --env-file=.env.local ./scripts/qa/telegram-agent-100-e2e.mjs`

~~~text
(node:13649) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
...
El script real, sin mocks SQL, queda escrito. La cobertura post-aplicación incluye replay por identidad aunque cambie `step_key`, negativo de delivery ownership, espejo por valor, pregunta durable, reject→re-staging, anti-self reject+authorize, receipt ownership, marcador contextual post-exec, turno mixto, contención con error específico y undo parcial que rehúsa todo sin alterar balance.

### 4.10 Dry-run E2E con modelo MOCK y PostgreSQL real

**NO EJECUTADO — PENDIENTE DE APLICACIÓN DE 116.**

El script queda escrito con completions 100% guionadas y cero OpenAI: lectura, write ordinario con transaction receipt, propuesta con manifest action exacta, auto-confirmación rehusada, confirmación posterior, settle y telemetría.

## 5. Qué NO se hizo y qué queda pendiente

No se hizo, deliberadamente:

- aplicar 116 ni ninguna otra migración;
- ejecutar M116.1–M116.6 o el dry-run dependiente;
...
- modificar 113 o las RPCs reutilizadas de 100/112/115;
- reescribir cuerpos de executors;
- borrar o debilitar planner, money/ownership/replay/undo/lock/read detectors;
- ejecutar gates en paralelo con mutaciones.

Pendiente de autorización explícita posterior al veredicto de Claude:

1. aplicar 116-r2;
2. ejecutar M116.1–M116.6 y dry-run MOCK;
3. repetir toda la batería sobre el schema aplicado.

## 6. Riesgos y objeciones

### 6.1 Confianza residual de `effect_mode` (aceptada en A3-4)

PostgreSQL deriva el marcador desde el enum recibido, pero capability→mode sigue viniendo del catálogo TypeScript bajo service role. No se añadió un catálogo SQL paralelo. Esta confianza residual fue aceptada por Claude y permanece explícita.

### 6.2 Composición del marcador post-exec con la paridad

La Adenda 3 exige a la vez espejo de staging y anexar el marcador `receipt`. Sin una equivalencia acotada, toda tool contextual money-moving quedaría verificada a nivel step pero fallaría inevitablemente el join de paridad del manifiesto. 116-r2 admite sólo la diferencia exacta de ese marcador server-owned. Se solicita revisión adversarial específica de esta inferencia de composición.

### 6.3 Recuperación depende de idempotencia durable ya existente

El resume de `executing` reejecuta `preflighted|applying` desde las filas persistidas. La seguridad contra doble write depende de los dedupes de writers y de `record_outcome`, tal como fija A3-2.6. El runner no reescribe executors ni inventa un segundo mecanismo.

### 6.4 Baseline `on` de Etapa 3

Tal como fija A3-4, el árbol es un híbrido v44+M0.11A en el camino `on`; no debe compararse contra `ce22319` puro en el head-to-head. Esta 1D no sustituye ese camino.

### 6.5 Warning de build

Build queda verde con el warning NFT preexistente causado por `readFileSync` en la página de capture. No se silenció ni se trató como producto de 1D.

### 6.6 Fallo de red del primer PostgreSQL

El primer intento no alcanzó Supabase (`ENOTFOUND`) y ejecutó 0/82. El reintento autorizado cerró 82/82 y verificó su limpieza; ambas salidas están íntegras en §4.

## 7. DDL propuesto — 116-r2 (NO APLICADO)
...
Estado: **NO APLICADO — pendiente de veredicto y autorización explícita.**
