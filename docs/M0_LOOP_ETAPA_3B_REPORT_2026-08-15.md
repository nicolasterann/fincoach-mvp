...
1. Autoridad de origen monetario, sólo en modo loop:
   - La cuenta origen elegida por el modelo ya no es autoridad, ni con un único candidato.
   - Se acepta únicamente si aparece en mensajes user-authored de la operación durable, si entra por el flujo S31 con confirmDefaultSource y coincide con defaultPaymentAccountId, si coincide con el vínculo durable de pago del fijo, o si una segunda delivery ya autorizó el manifiesto.
   - Se cubren fromAccount, sourceAccountId, accountId saliente, paymentSourceAccountId, newSourceAccountId y los movimientos batch.
   - El matching es tipado: la aparición de “Produbanco” dentro del nombre de tarjeta “Produbanco MV” no prueba que el usuario nombró la cuenta.
   - El branch de ejecución y los guards de KIPU_AGENT_MODE=on no se modifican. La registry compartida sí gana los dos modos exigidos por A6-2; la implicación para una futura corrida local de on queda explicitada en §6.

2. Segunda delivery por modo:
   - CONDITIONAL_SECOND_DELIVERY_RULES pasa de 8 a 10.
   - Se agregan unrecorded_capital_return y unrecorded_borrowed_funds para record_person_payment direction=in.
   - loan_repayment registrado continúa inmediato.

3. Hard output guard observable:
   - Los tres rechazos se clasifican exactamente como technical_structure_leak, saldo_not_publishable o deterministic_voice_rejected.
   - El diagnóstico queda en loopDiagnostic y en advisories/metadata.
   - La reproducción focal de ME2 probó que el patrón pesado heredado rechazaba “Ojo:” aun siendo una respuesta factual válida. El loop usa un guard determinista mínimo para voseo inequívoco; el camino on conserva el guard anterior sin cambios.

4. Costo durable:
   - El runner persiste después de cada completion un snapshot atómico de tokens y USD real acumulado.
   - El launcher detached crea y publica el usagePath, y lo pasa al worker mediante M0_LOOP_USAGE_STATUS_PATH.
   - Un aborto conserva todo uso asentado antes de la señal.

5. Red:
   - IR330a fija candidato único, autoridad user-authored, colisión cuenta/tarjeta, S31 y vínculo fijo.
   - IR330b fija los dos modos no registrados y el repago registrado inmediato.
   - IR330c fija la reproducción focal ME2 y las tres causas tipadas del hard guard.
   - IR330d fija el snapshot streaming de costo.
   - IR330e fija la presencia de la delivery actual en el advisory ligero de cifras.
   - M0M504 y M0M505 muerden autoridad de origen y segunda delivery.
   - El dry-run añade DRY_ORIGIN y DRY_CAPITAL: 5/5 recorridos completos.

No hubo desviaciones del contrato.

## 2. Archivos creados/modificados

- src/lib/ai/agent/agent-operation-authority.ts — dos reglas condicionales server-side por modo.
- src/lib/ai/agent/kipu-agent-tools.ts — guard loop-only de autoridad de origen y resolución tipada cuenta/tarjeta.
- src/lib/ai/agent/kipu-agent-loop.ts — diagnóstico del hard guard y evidencia ligera que incluye la delivery user-authored.
- src/lib/ai/voice-policy.ts — guard determinista mínimo específico del loop; el guard del envelope/on queda intacto.
- scripts/qa/m0-loop-conversation-e2e.mjs — espejo 10/10, DRY_ORIGIN, DRY_CAPITAL y snapshots de costo streaming.
- scripts/qa/run-m0-loop-conversation-background.mjs — usagePath durable comunicado por launcher/worker/status.
- src/app/dev/capture-test/page.tsx — IR329 actualizado e IR330a–e.
- scripts/qa/telegram-agent-regression-audit.mjs — M0M504 y M0M505.
- docs/M0_LOOP_ETAPA_3B_REPORT_2026-08-15.md — esta acta.

## 3. Decisiones de diseño

1. Identidad de cuenta por spans tipados. Se reutiliza la normalización de entidades, pero una coincidencia de cuenta cubierta por una entidad tipada más larga no cuenta como autoridad. Esto evita que una tarjeta cuyo nombre contiene el de la cuenta autorice un origen no dicho, sin regex sobre intención o capability.

2. El guard de origen vive antes de los guards anteriores sólo en executeTool con mode=loop. Así, una falta de autoridad produce staging por el dispatcher existente y no reescribe ningún executor.

3. La excepción del fijo exige igualdad por id con una fila activa, paymentSourceType=account y paymentSourceId exacto. No hay adopción por descripción.

4. La excepción S31 exige a la vez confirmDefaultSource=true y que la cuenta propuesta sea el defaultPaymentAccountId de la tarjeta seleccionada. Un default de moneda o un único candidato no alcanza.

5. Diagnóstico focal ME2. El viejo patrón determinista marcaba como voz inválida cualquier apertura “Ojo:” y sustituía un read correcto por continuidad. En el loop, calidad/tono pertenece al juez graduado; el hard guard conserva sólo la violación inequívoca “vos”. El guard v44/on no cambió.

6. Persistencia de costo. Se escribe primero usagePath.tmp y luego se renombra sobre usagePath, en el mismo directorio, después de cada addUsage. El archivo siempre representa una frontera de completion completa.
...
Dry-run MOCK: ejecuta 5 recorridos representativos y valida estáticamente los 50 contratos del catálogo.
...
[DRY_SENSITIVE] plomería propuesta y confirmación sensible
  ...
  CONDUCTA PASS
  CALIDAD  {"average":5,"correct_numbers":5,"human_coach_voice":5,"model":"MOCK","only_necessary_questions":5,"rationale":"Juez MOCK: transcript enlatado y shape completo validados.","resolved_request":5}
  TRANSCRIPT [{"assistant":"Preparé la creación de Ahorro MOCK. ¿Confirmas que la cree?","user":"Agrega una cuenta nueva llamada Ahorro MOCK en USD."},{"assistant":"Listo, creé la cuenta Ahorro MOCK en USD.","user":"Sí, crea exactamente esa cuenta."}]
...
loopUsage agregado: {"cachedInputTokens":520,"calls":13,"inputTokens":1300,"outputTokens":260}
Judge usage agregado: {"cachedInputTokens":0,"calls":5,"inputTokens":900,"outputTokens":225}
Paraphrase usage agregado: {"cachedInputTokens":0,"calls":0,"inputTokens":0,"outputTokens":0}
Costo real acumulado: 0.006700 USD
Calidad promedio: 5.00/5
Costo estimado corrida completa: {"baseline":"native loop","basis":"MOCK token telemetry plus a deterministic paraphrase allowance, scaled to the complete catalog","estimatedTurns":107,"estimatedUsd":0.1,"ratesUsdPerMillion":{"coach":{"cached":0.25,"input":2.5,"output":15},"mini":{"cached":0.1,"input":0.4,"output":1.6}},"scenarios":50,"tokens":{"agent":{"cachedInputTokens":7949,"calls":199,"inputTokens":19871,"outputTokens":3974},"judge":{"cachedInputTokens":0,"calls":50,"inputTokens":9000,"outputTokens":2250},"paraphrase":{"cachedInputTokens":0,"calls":1,"inputTokens":677,"outputTokens":736}}}
M0 tres carriles (loop, MOCK): 5/5 duros verdes
~~~

Exit: 0.

El valor USD 0.006700 es la valoración sintética de los tokens MOCK para probar el acumulador; no corresponde a gasto facturado. Gasto pagado de esta ronda: USD 0.
...
### 4.4 Build — intento sandbox y reintento final autorizado

Primer comando: npm run build
...
Exit: 1, causa ambiental NETWORK_SANDBOX/GOOGLE_FONTS.

Reintento con red autorizada: npm run build
...
✓ Generating static pages using 11 workers (36/36) in 246ms
...
[kipu.route] {"ts":"2026-08-15T16:55:35.133Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":false,"transactionType":"fixed_expense_create"}
[kipu.route] {"ts":"2026-08-15T16:55:35.133Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":true,"transactionType":"fixed_expense_create"}
[kipu.route] {"ts":"2026-08-15T16:55:35.133Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":false,"transactionType":"fixed_expense_create"}
835/835 capture checks
(node:94056) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/capture/capture-matching.ts is not specified and it doesn't parse as CommonJS.
...
El primer intento encontró que el smoke IR297 buscaba una cadena genérica que el helper nuevo también contenía, por lo que M0M432 sobrevivía sin que su frontera real hubiera regresado. Se acotó IR297 al cuerpo de guardCorrectiveToolCallWith. No se cambió el mutante ni producto. La corrida final íntegra fue:
...
~~~text
(node:94060) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
...
- No se modificó ni aplicó DDL. La migración 116-r3 aplicada por el founder permanece intacta.
- No se hizo commit, push ni deploy.
- No se ejecutaron muestras pagadas ni se llamó al juez real; toda la validación conversacional fue MOCK.
- No se modificó el branch KIPU_AGENT_MODE=on ni se reescribieron cuerpos de executors; tampoco hubo deploy a producción.
- No se lanzó todavía la re-medición autorizada. Tras el veredicto de Claude corresponde una corrida loop que reemplaza loop-1 y luego loop-2, con los criterios de avance/abandono originales.
- El acumulado real previo de Etapa 3 es aproximadamente USD 13.13; esta ronda agregó USD 0 facturados.

## 6. Riesgos y objeciones

1. Colisión de nombres tipados detectada durante el dry-run. El primer guard reconocía “Produbanco” dentro de “Produbanco MV” como cuenta nombrada. Eso habría reabierto exactamente ME3. Se corrigió en la frontera de identidad con spans tipados y queda fijado por IR330a.

2. Evidencia del advisory ligero. El primer DRY_CAPITAL consumió una completion extra porque deterministicEvidence contenía contexto y receipts, pero no el mensaje actual que acababa de declarar 83.86. Añadir input.message al conjunto de presencia es necesario para el chequeo ligero; no le concede autoridad semántica ni de entidad al texto.

3. Guard de voz del loop. Reducir el hard veto determinista a “vos” permite otros defectos de voz posibles, pero ese riesgo ya pertenece al carril graduado de CALIDAD. Mantener el regex pesado habría reintroducido un cerrojo binario y sustituyó una respuesta financiera correcta en la reproducción ME2. El camino v44/on retiene el comportamiento anterior.

4. Persistencia ante señales. El snapshot garantiza costo hasta la última completion incorporada por addUsage. Una muerte no capturable entre la respuesta del proveedor y addUsage todavía podría perder esa única completion; cerrar esa ventana exigiría instrumentar la llamada de proveedor o un ledger externo. Para SIGTERM normal y abortos entre escenarios, el contrato queda cubierto.

5. El dry-run imprime USD 0.006700 porque aplica tarifas a telemetría MOCK para probar el cálculo. No es una factura. El reporte distingue explícitamente costo modelado de gasto real.

6. El warning NFT del build es preexistente y no se ocultó. El build final termina exit 0.

7. Precisión sobre on. CONDITIONAL_SECOND_DELIVERY_RULES es una registry compartida que también alimenta el contrato del planner v44+M0.11A. Por tanto, sumar allí los dos modos —mandato textual de A6-2— haría que una futura ejecución local de on también los valide como segunda delivery, aunque no se tocó el branch on ni se desplegó nada. Si “on intacto” pretendía excluir incluso este efecto compartido, existe una tensión factual con “entran a CONDITIONAL_SECOND_DELIVERY_RULES”; no se inventó una registry paralela y se canaliza aquí para ratificación de Claude.

No hay objeción bloqueante ni desvío unilateral.
