...
# M0 — Etapa 3G / ronda 1Y — Cohesión de conjunto en la frontera loop

Fecha: 2026-08-16  
Árbol: local, sin commit  
Contrato: ADENDA 14 / 1Y  
Resultado: implementación completa; gates verdes; cero llamadas pagadas.

## 1. Alcance implementado frente al contrato 1Y

Se implementó la garantía observable pedida: si cualquier acción económica de un turno queda manifest-bound, ninguna escritura económica de ese turno se ejecuta fuera del manifiesto. La garantía cubre tool calls en una misma completion, repartidos entre completions y en cualquier orden, incluido un write que individualmente era inmediato antes de que aparezca la primera acción manifest-bound.

La dirección inversa también queda preservada: un turno compuesto sólo por acciones económico-inmediatas se ejecuta sin confirmación nueva. DRY_WRITE sigue escribiendo el café, ahora después de conocer el conjunto completo del turno y antes de la narración definitiva basada en sus receipts reales.

No hubo desvíos del contrato. No se modificaron el camino envelope/on, writers SQL, migraciones 117/1X, cuerpos de executors ni DDL. No se añadió routing por frases.

## 2. Archivos creados o modificados en 1Y

- src/lib/ai/agent/kipu-agent-loop.ts — clasificación económica por completion, cola diferida por turno, promoción cohesiva a manifiesto y narración posterior a receipts.
- src/lib/ai/agent/kipu-agent-tools.ts — preflight económico sin ejecutar el executor y permit request-local para consumir exactamente ese veredicto al ejecutar.
- scripts/qa/m0-loop-conversation-e2e.mjs — DRY_SET_COHESION, mocks post-receipt y aserciones de cero filas / manifiesto completo / paridad verificada.
- src/app/dev/capture-test/page.tsx — IR335 y ajustes de pins vivos afectados por el refactor.
- scripts/qa/telegram-agent-regression-audit.mjs — M0M522 y M0M523; reanclaje cardinal de M0M497/M0M510 sin cambiar su conducta.
- docs/M0_LOOP_ETAPA_3G_REPORT_2026-08-16.md — acta 1Y con prueba de orden y salidas íntegras.

## 3. Decisiones de diseño y prueba de orden

### 3.1 Mecánica elegida

Cada completion se clasifica completa antes del despacho de su primer tool call. Toda acción con effect mode economic_event o contextual_event se stagea primero y queda en una cola request-local; todavía no toca su writer.

Al cierre sin tool calls, el loop ejecuta dos fases seriales:

1. preflight de todas las acciones diferidas, sin entrar en ningún executor;
2. sólo si todas siguen siendo inmediato-elegibles, ejecución en seq usando permits request-local. Los receipts reales se asientan y se agregan como evidencia de sistema antes de pedir la narración final.

Si el lote, el dispatcher o un preflight descubre una obligación de manifiesto, todas las acciones económicas diferidas se promueven al mismo conjunto stageado. Ningún writer económico había corrido todavía.

### 3.2 Demostración por orden adversarial

- Inmediato antes de manifest-bound, en completions distintas: el primero queda diferido. Al aparecer el segundo, promoteDeferredEconomic incorpora el primero al manifiesto.
- Manifest-bound antes de inmediato: stagedSensitive ya está activo y el inmediato se stagea en el mismo conjunto.
- Ambos en una completion: loopCompletionEconomicCallIds clasifica el lote completo antes del primer dispatch.
- Manifest-bound descubierto dentro del guard: el preflight de todos ocurre antes de consumir cualquier permit; el primer requisito promueve el conjunto entero.
- Turno sólo inmediato: tras cerrar el turno sin manifest-bound, consume permits en orden, asienta receipts y recién entonces compone la respuesta.
- Modelo que divide calls entre N completions: la cola deferredEconomic vive durante todo el loop del turno, no por completion.

El permit contiene stepKey, capability, argumentos ya materializados y serverAuthorized. Sólo puede consumirse por el mismo step/capability y nunca persiste como autoridad durable. Los writers y sus locks/replays siguen siendo la última autoridad.

### 3.3 Frontera de vínculo tardío

register_card_payment sin fromAccount/sourceAccountId se clasifica mecánicamente como manifest-bound antes de ejecutar: el writer exigirá ese vínculo y no puede descubrirlo después de un write vecino. La decisión inspecciona argumentos tipados y capability, nunca texto de usuario. La autoridad de fuente sigue viniendo de mensajes user-authored, S31 o vínculo durable.

### 3.4 Red y cardinales

- Capture previo: 851. IR335 agrega una aserción: 851 + 1 = 852.
- Mutaciones previas: 515. M0M522 + M0M523: 515 + 2 = 517.
- IR335 fija clasificación del lote antes del loop de calls, diferimiento, promoción total, preflight completo antes de permits, receipt-before-reply, DRY_SET_COHESION y dirección inversa DRY_WRITE.
- DRY_SET_COHESION reproduce ME4: pregunta de origen → respuesta que lo resuelve → un pago temprano inmediato-elegible → capital y dos pagos manifest-bound en una completion posterior → cero filas → un manifiesto de cuatro acciones → una confirmación → cuatro writes exactos y manifiesto verified.

## 4. Salidas íntegras de gates

Los gates se ejecutaron en orden serial. Mutaciones corrió sola, sin ningún otro gate o auditor. Los exit codes se leyeron directamente, sin pipes.
...
~~~text
(node:67618) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
...
Exit: 0. Resultado: 3/3; residuo cero según el cleanup del harness.

### 4.2 Dry-run MOCK loop — 15 patas
...
Dry-run MOCK: ejecuta 15 recorridos representativos y valida estáticamente los 50 contratos del catálogo.
...
loopUsage agregado: {"cachedInputTokens":2480,"calls":62,"inputTokens":6200,"outputTokens":1240}
Judge usage agregado: {"cachedInputTokens":0,"calls":15,"inputTokens":2700,"outputTokens":675}
Paraphrase usage agregado: {"cachedInputTokens":0,"calls":0,"inputTokens":0,"outputTokens":0}
Costo real acumulado: 0.030680 USD
Calidad promedio: 5.00/5
Costo estimado corrida completa: {"baseline":"native loop","basis":"MOCK token telemetry plus a deterministic paraphrase allowance, scaled to the complete catalog","estimatedTurns":107,"estimatedUsd":0.12,"ratesUsdPerMillion":{"coach":{"cached":0.25,"input":2.5,"output":10},"mini":{"cached":0.1,"input":0.4,"output":1.6}},"scenarios":50,"tokens":{"agent":{"cachedInputTokens":9477,"calls":237,"inputTokens":23693,"outputTokens":4739},"judge":{"cachedInputTokens":0,"calls":50,"inputTokens":9000,"outputTokens":2250},"paraphrase":{"cachedInputTokens":0,"calls":1,"inputTokens":677,"outputTokens":736}}}
M0 tres carriles (loop, MOCK): 15/15 duros verdes
~~~

Exit: 0. Resultado: 15/15; `DRY_SET_COHESION` prueba cero filas antes de la confirmación única y manifiesto completo; `DRY_BORROWED_LINK` cruza proposal → confirm → write caja+deuda → step verified → manifest verified → respuesta con receipts; residuo cero. Modelo MOCK: gasto facturado USD 0. El `0.030680 USD` impreso es valoración sintética de la telemetría MOCK.
...
[kipu.route] {"ts":"2026-08-16T05:02:37.609Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":false,"transactionType":"fixed_expense_create"}
[kipu.route] {"ts":"2026-08-16T05:02:37.609Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":true,"transactionType":"fixed_expense_create"}
[kipu.route] {"ts":"2026-08-16T05:02:37.610Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":false,"transactionType":"fixed_expense_create"}
852/852 capture checks
(node:80297) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/capture/capture-matching.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use node --trace-warnings ... to show where the warning was created)
~~~

Exit: 0. Resultado: 852/852.
...
### 4.7 Mutaciones — corrida final aislada
...
M0 mutations: 517/517
~~~

Exit: 0. Resultado: 517/517. M0M522 y M0M523 murieron por IR335; cero crashes; todos los anchors cardinales valieron 1. La corrida final corrió sola.
...
~~~text
(node:83473) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
...
- No hubo llamadas a modelos pagados. El único modelo de la batería fue MOCK.
- No se ejecutó la CORRIDA FINAL 3; queda bloqueada hasta el OK explícito de Claude.
- No se creó, aplicó ni modificó DDL. Las migraciones 117 y su schema vivo no cambiaron.
- No se tocó KIPU_AGENT_MODE=on, producción, writers SQL ni cuerpos de executors.
- No hubo commit, push ni deploy.
- No se escribió contra la cuenta real del founder; todos los E2E usaron personas desechables con cleanup por identidad y residuo cero.
- No queda implementación pendiente dentro de 1Y.

## 6. Riesgos nuevos y objeciones

### 6.1 Preflight request-local y cambios concurrentes

El permit evita repetir el guard entre preflight y executor, pero no intenta sustituir las pruebas durables del writer. Una mutación concurrente de catálogo/estado entre ambas fases puede hacer que el SQL rehúse; no puede autorizar un write que el SQL no admita. Es el fail-closed correcto y no requiere DDL.

### 6.2 Guardia específica de vínculo tardío

La regla register_card_payment sin fuente es mecánica y necesaria para que una precondición conocida del writer no aparezca post-confirmación. Si en el futuro otro writer incorpora un vínculo obligatorio que no esté visible por su schema/guard antes del executor, deberá entrar al mismo inventario tipado. IR335 evita que se resuelva ejecutando primero.

### 6.3 Incidente de auditoría por sesión compactada

Una corrida de mutaciones perdió su handle de terminal durante compactación pero siguió viva. Un segundo intento alcanzó el handshake capture durante la ventana temporal de un mutante y se negó a correr con 851/852, exactamente como exige M0M333. Causa: HARNESS_CONCURRENCY/MUTANT_WINDOW_OBSERVED. Se verificó por proceso que sólo quedaba el mutador original, se esperó su restauración, capture volvió a 852/852 y la corrida final retenida cerró 517/517 sola. No se cambió producto ni detector por este incidente.

### 6.4 Reanclajes del catálogo

M0M497 y M0M510 conservaron sus flips conductuales, pero sus literales from dejaron de existir tras el refactor. Se reanclaron al sitio vivo con cardinalidad 1. Ambos murieron por sus checks originales; no se alteraron los detectores de money, ownership, replay, undo, locks o lecturas.

No tengo una objeción arquitectónica que impida auditar 1Y.

## 7. DDL propuesto

Ninguno. 1Y es TS/harness-only y no requiere migración.
