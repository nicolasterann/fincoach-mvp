...
- [x] Stage 17 boundaries: did NOT begin Stage 18; eToro/live brokerage sync, live market prices, deep retirement/tax modeling and a dedicated onboarding-UI flow are explicitly DEFERRED; the ONLY migration (025) is additive and NOT applied; did NOT commit/push/deploy.
...
7. [x] Stage 17 — Goals, Mini-Goals, Wealth Builder & Human Financial Priorities (**code-complete 2026-06-17**; migration 025 created, NOT applied; not committed/deployed) — multiple goals + priority portfolio, impulse-safe mini-goals, human-realistic priority-aware allocation (never 100% to debt, controlled joy), opportunity cost, goal feasibility, investment/compounding, net worth + wealth target, goal-aware cashflow/Margen recarve, goal/mini-goal ambient autopilot
...
Stages 1–16 are production-live. Stage 17 (Goals / Wealth Builder, gate **95/95**) is **code-complete** (lint + build green, adversarial review run + fixes applied) but not yet committed/deployed, and adds **one additive migration (025) that is NOT yet applied**. Stage 17 makes Kipu the user's personal system for turning money into goals: multiple goals + a priority portfolio, **impulse-safe mini-goals** ("puedes comprarlo hoy, pero te propongo $25/sem por 4 semanas y lo tienes sin tocar tu viaje ni tu tarjeta"), a human-realistic allocation engine (never 100% to debt, controlled joy preserved), opportunity-cost reasoning, goal feasibility, investment/compounding, net worth + a wealth target, and goal/mini-goal Telegram autopilot. Committed contributions reserve money through the SAME Margen recarve (no double counting); all reads degrade gracefully so production is unchanged until both migration 025 is applied and the code is deployed. Gated behind the normal rollout step: apply 025 + commit + push + deploy + smoke validation.
