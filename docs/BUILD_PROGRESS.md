...
| Objetivo mensual — segunda ronda de review del founder (Bloque H) — ocho puntos: **(1)** el onboarding no creaba la primera versión → todo usuario nuevo tenía su primer mes sin versión y se recalculaba con el monto actual; ahora `save-actions` siembra `objective_versions` con el mes del CLIENTE. **(2)** los meses previos a la primera versión caían al monto actual MUTABLE → `objectiveForMonth` ahora resuelve a la versión MÁS ANTIGUA (inmutable); solo sin ninguna versión cae al actual — recién ahora es verdad que «un cambio no reescribe un mes pasado». **(3)** el cambio de objetivo eran dos escrituras no atómicas con el resultado de la 2ª ignorado → RPC `kipu_upsert_budget_objective` (migración 053): puntero + versión en UNA transacción, y si falla NO se confirma éxito. **(4)** `loadObjectiveVersions` devolvía `[]` tanto sin historia como ante un fallo de Supabase → ahora `{ok, rows}`; con la lectura caída el motor camina SOLO el mes corriente (nunca remide el pasado con el objetivo de hoy) y el cierre se salta al usuario (un cierre es permanente). **(5)** el hipotético corregía el Saldo pero `evaluateAdvisoryDecision` seguía pesando el precio COMPLETO → recomendación contradictoria («ni toca tu Saldo» + «mejor no»); ahora la ruta cash pesa el costo incremental (y el caso totalmente absorbido responde directo), la de tarjeta mantiene el monto completo porque su veredicto es por presión de deuda. **(6)** el FX podía reescribir el pasado (versiones nativas revaluadas a tasa viva vs transacciones con su `base_amount` histórico) → `amount_base` congelado al decidir: meses pasados usan el congelado, solo el corriente revalúa vivo; backfill de las filas sembradas. **(7)** `category` de `evaluate_purchase` documentada como obligatoria pero no requerida ni tipada → ahora `required` + enum. **(8)** el seed de 052 usaba el mes del server → corregido a `now() at time zone` del usuario (join `user_engagement`) para replays/ambientes nuevos. Huecos de pruebas cerrados: H.20 (fallback inmutable), H.21 (lectura fallida no reescribe el pasado), H.22 (`executeEvaluatePurchase` end-to-end: resumen y recomendación coherentes). Gate **279/279** · lint/build/tsc limpios · repro founder sin cambios (Comida 317.96/333.33, Saldo 139.69; congelado 333.33 verificado en los tres regímenes). | H-fix2 | 053 | live |
| Objetivo mensual — fixes de consistencia del founder (Bloque H) — cinco puntos corregidos: **(P1-1) cambiar el objetivo ya no reescribe el pasado**: el objetivo se VERSIONA por mes (`objective_versions`, migración 052, seed del mes actual); el walk multi-mes mide cada mes contra el objetivo VIGENTE entonces y el cierre reporta el mes cerrado con SU objetivo (antes, subir el objetivo en julio borraba el exceso de junio que ya había drenado el Saldo, y el tanque subía retroactivamente); `update_budget_category` estampa la decisión en el mes del USUARIO. **(P1-2) compra hipotética que cruza parcialmente**: helper puro `objectiveDrainForPurchase` + `evaluate_purchase` acepta `category` y descuenta SOLO el exceso incremental (objetivo 500, llevas 480, compra 50 → salen 30, ni 50 ni 0); prompt con los tres casos. **(P1-3) mes del usuario vs mes del server**: `computeBudgetProgress` acepta `todayISO`+`toDayISO` y deriva mes/día/días-restantes/filtro del calendario del USUARIO (antes mezclaba mes-UTC del server con números tz-usuario del objetivo justo en el rollover; Vercel corre en UTC). **(P2-4) guard de corrección**: `correct_movement` exige objetivo activo + categoría de objetivo antes de aplicar `newBudgetTreatment='saldo'`; si no, se niega honestamente en vez de confirmar un no-op. **(P2-5) simplicidad**: el cierre dice UNA comparación + UNA pregunta (la mecánica —exceso drenado, extraordinarios— queda como DETALLE solo si el usuario pregunta) y el pie de Gasto baja a una línea. Gate **276/276** (H.16–H.19 nuevas) · lint/build/tsc limpios · repro founder sin cambios (Comida 317.96/333.33, no cruzado, Saldo 139.69). | H-fix | 052 | live |
| Objetivo mensual (Bloque H) — **doctrina comida/transporte**: la comida (súper/restaurante/delivery/café) y el transporte cuentan contra un **OBJETIVO mensual que el usuario DECIDE** (no un estimado auto-ajustado). Dentro del objetivo NO drena el Saldo (ya reservado vía essentialEstimate → fillDaily); al cruzarlo, SOLO el exceso drena el tanque día a día (acumulador **multi-mes** para que el exceso de fin de mes envejezca en la ventana de 40 días como cualquier gusto, sin saltar al cambiar de mes). Un **extraordinario** confirmado (`budget_treatment='saldo'`) sale directo del Saldo, no consume objetivo ni entra en la comparación del cierre; jamás se marca sin confirmación del usuario o instrucción permanente (el tool exige objetivo activo o registra normal sin mentir). **Refund hereda** el registro del original (objetivo↔Saldo). Excluidos del acumulador: fijos (recurringExpenseId), cuotas (installment) y comida en viaje (travel); alcohol solo → su categoría, alcohol en comida → comida. **Señal de ritmo pre-cruce** (REQUERIDA) en home/spending/digest/ambient («a este ritmo lo cruzas el N»). **Cierre mensual** (cron día 1-3 tz-usuario, idempotente por mes): reporte con lo aprendido (objetivo X, cerraste en Y — desborde incluido; extraordinarios aparte; se distingue lo realmente drenado del desborde del seed), **sobrante → Reservas por defecto** (no-write; `resolve_objective_close` para redirigir sin mover plata sola). El objetivo se deriva de `budget_categories` (sin fila → comportamiento legacy exacto, rollout seguro). Onboarding pide «objetivo mensual» para comida/transporte. Motor puro `objectives.ts` + `objective-closes-store.ts` + `objective-month-close.ts`; migración **051** (`transactions.budget_treatment` + `objective_month_closes` + RPC `kipu_apply_ledger_entry` reemplazado). Red team multi-agente (8 dimensiones, 48 hallazgos → verificación adversarial): **13 root-causes reales corregidos** (doble-conteo del rollover, seed reportado como drenado, cadena de escritura de refund/corrección, cierre robusto ante errores de query, cold-start del onboarder, honestidad del 'saldo' sin objetivo, paginación del loader, superficies). Gate **272/272** (H.1–H.15) · lint/build/tsc limpios · repro con datos reales del founder (Comida 317.96/333.33, no cruzado, cruce proyectado, Saldo intacto). | H | 051 | live |
| Tesorería fixes (Bloque F, beta del founder) — cinco bugs corregidos: (1) la Tesorería ya NO reusa el horizonte corto del runway (días-al-próximo-sueldo) sino un **calendario propio de 45 días** que captura el próximo vencimiento de cada obligación (tarjeta, arriendo, préstamos), así los pisos dejan de ignorar pagos a semanas vista; (2) cuentas tipo **cash** son spend-through: nunca generan piso-colchón, nunca aparecen «cortas», nunca son destino/ancla de un movimiento (piso ≤ su saldo); el colchón de gasto solo aplica a cuentas con share≥15% o la top no-cash (un gasto suelto ya no crea piso); (3) el **bolsillo muerto** solo se drena a un ancla de la MISMA moneda y no-cash (nunca recomienda convertir USD→ARS ni «mover a efectivo»); (4) el **ideal** se muestra con contexto («lo ideal sería X acá» solo si difiere; si no, «está bien acá»/«sin pagos propios pronto»); (5) **voz tú** en página + digest + señal + ambient, y cada cuenta se muestra en su **moneda nativa** (pesos para cuentas ARS); (6) **movimientos schedule-aware (Opción 1 del founder)**: la cuenta corta ya NO se pide «mové el total del ciclo antes de la primera fecha» — cada cuenta lleva un `shortfallSchedule` de tramos por vencimiento (Visa 744 el 22, créditos 313 el 5 del mes siguiente) y el movimiento urge SOLO el próximo tramo (744 el 22), con el resto como «y luego … · o mueve 1.056 de una vez». El earmark sigue siendo el total (la plata de tramos futuros NO cuenta como libre); Σtramos = −surplus. Red team (2 pasadas): 3 hallazgos de la 1ª (cash-ancla, «sus pagos del mes» usted, exclusión-cash inconsistente) → 3 corregidos. Gate 257/257 · repro con datos reales del founder (Pichincha 744 urgente + 1.056 total, Efectivo sin alerta, Supervielle ideal=saldo). **(7) pulido de copy:** las etiquetas de obligación se muestran limpias (`cleanObligation` quita el sufijo redundante «(pago del mes)» → se acabaron los paréntesis-dentro-de-paréntesis en la home card, el move card y el digest) y se eliminó todo voseo argentino de la copy («mové/despreocupás/de una» → «mueve/te olvidas/de una vez»; «tenés» → «tienes»): voz tú neutra LatAm en todas las superficies. | F-fix | — | live |
