...
- [x] Stage 14 boundaries: did NOT begin a later stage; did NOT reopen Stage 12/13 (extended the statement extractor only for date-awareness, the Stage 14 mandate); did NOT commit/push/deploy or apply migrations.
...
4. [x] Stage 14 — Card & Debt Protection / Interest Intelligence (**code-complete 2026-06-16**; migration 023 created NOT applied; not committed/deployed) — date-aware obligations, debt-health model, interest/payoff/debt-vs-investment intelligence, protective Telegram nudges, all AI-first and money-safe

The product has moved from establishing financial truth (Stage 11–12) to keeping it fresh without effort (Stage 13) to actively protecting the user from debt, card and interest mistakes (Stage 14).

### Immediate next milestone

Earlier phases through Phase 10.6 and AI-native Stages 1–13 are complete and production-live. Stage 14 (card/debt/interest protection) is **code-complete** (deterministic gate 60/60, lint + build green): date-aware obligations that an older statement can't overwrite, a per-card debt-health model, honest interest/payoff/debt-vs-investment intelligence, protective Telegram nudges on the Stage 13 loop, and a debt-health dashboard surface — all AI-first and money-safe. It is gated behind ONE manual step before deploy: apply migration `023_stage14_debt_protection.sql`. Until then production is unchanged (debt reads degrade gracefully).

The dashboard/UI is approved as a strong first version. Margen Kipu and Pulso Kipu are the central product concepts. Capture works across web text, Telegram text/voice/photo/PDF statements with idempotent, resumable statement import; Kipu proactively checks in on Telegram when useful; and Kipu now understands cards/debts/interest well enough to protect the user from payment, interest and allocation mistakes.

The next major stage is:

- Stage 15 — (to be defined) on top of the now-complete protection layer.

Stage 14 delivered the debt/card/interest guardian on top of Stage 12's statement import and Stage 13's freshness/ambient loop: date-aware obligations, payment-readiness, minimum-vs-full and interest intelligence, payoff strategy, debt-vs-investment guidance, and protective (never shaming) nudges.
