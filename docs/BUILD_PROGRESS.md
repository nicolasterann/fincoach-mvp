...
#### Stage 16 — Budget Intelligence, Category Learning & Behavioral Spending OS — IMPLEMENTED (migration 024 created, NOT applied; not committed/deployed, 2026-06-17)

- [x] **Stage 16 — the spending-behavior brain: code-complete, AI-first, "genius inside, simple outside."** Deterministic code computes a rich behavioral model of the user's spending; the AI answers SIMPLY (what changed / what matters / what to do / how it affects this week). It is NOT a 30-category budgeting module — it learns the user's own "normal" and only surfaces the few things that matter, always tied back to safe spend, never shaming. Built ON Stage 15 (cashflow safe-spend) and the existing ledger; numbers come from the engine, never the LLM.
...
- [x] Stage 16 migration `024_stage16_merchant_memory.sql` (additive, NOT applied): `user_merchant_memory` (per-user learned merchant facts keyed by normalized match pattern; unique (user_id, match_pattern) so corrections upsert; RLS default-deny, service-role grant). No existing object dropped or weakened.
...
- [x] Stage 16 boundaries: did NOT begin Stage 17+; did NOT create a second user-facing concept (budget intelligence ties back to Margen/cashflow); the ONLY migration (024) is additive and NOT applied; did NOT commit/push/deploy.
...
6. [x] Stage 16 — Budget Intelligence, Category Learning & Behavioral Spending OS (**code-complete 2026-06-17**; migration 024 created, NOT applied; not committed/deployed) — category intelligence (no double counting), merchant normalization + learning memory, learned per-category baselines, dynamic non-shaming budgets, subscription detection, graded anomalies, margin attribution, behavioral-insight synthesis; surfaced as the few things that matter, tied to safe spend

The product moved from establishing financial truth (11–12) → keeping it fresh (13) → protecting from debt/interest mistakes (14) → understanding the user's whole cashflow and collapsing it into one calm daily truth (15) → learning the user's spending behavior and surfacing only what matters, without shame (16).

### Immediate next milestone

Stages 1–14 are production-live. Stage 15 (cashflow autopilot, gate 65/65) and Stage 16 (behavioral spending OS, gate **79/79**) are both **code-complete** (lint + build green, adversarial reviews run) but not yet committed/deployed. Stage 16 is the AI-first spending-behavior brain: deterministic modules learn the user's own "normal" (category baselines, merchant families + learning memory, dynamic budgets, subscriptions, graded anomalies, margin drivers) and a synthesis layer picks THE one thing worth saying; the AI answers SIMPLY (what changed / what matters / what to do / how it hits this week), never shaming, never double-counting, always tied back to Margen/cashflow. It adds **one additive migration (024 `user_merchant_memory`) that is NOT yet applied** — all reads degrade gracefully so production is unchanged until both the migration is applied and the code is deployed. Gated behind the normal rollout step: apply 024 + commit + push + deploy + smoke validation.

The dashboard/UI is approved. Margen Kipu and Pulso Kipu remain the central concepts. Kipu now captures across channels, keeps data fresh, protects against debt/interest mistakes, understands the user's cashflow, and learns their spending behavior — answering "¿en qué se me va?" / "¿por qué bajó mi margen?" / "¿dónde recorto?" / "¿qué suscripciones tengo?" as simply and kindly as "¿cuánto puedo gastar?".

The next major stage is:

- Stage 17 — Goals / Wealth Builder — Stage 16's learned baselines + cashflow safe-spend are a clean foundation for goal feasibility and saving-rate intelligence.

Following stages (mapped, not built): Stage 17 — Goals / Wealth Builder; Stage 18 — Personalization, Memory & Life Context; Stage 19 — Household / Shared Finance; Stage 20 — Scale, Monetization & Reliability.
