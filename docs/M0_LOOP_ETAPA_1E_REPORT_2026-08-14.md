# M0 Tool Loop — Etapa 1E — reporte pre-aplicación
...
Se leyó completa la ADENDA 4 (A4-1–A4-4) de `docs/M0_CLAUDE_AUDIT_TOOL_LOOP_2026-08-14.md` antes de modificar código. Esta micro-ronda implementa únicamente el delta 1E y mantiene el loop detrás de `KIPU_AGENT_MODE=loop`.

Alcance entregado:

1. **116-r3 — verificación cruzada de versiones.** `kipu_verify_agent_loop_step` ya no compara `v_op.plan_version` con la versión del step. Conserva ownership de operación, status `verifying`, plan `{"mode":"loop"}`, lease exacto/vivo y la búsqueda de la fila por `operation_id + user_id + plan_version + step_key`; capability, fingerprint y arguments siguen ligados por valor. `kipu_verify_agent_loop_manifest` sí conserva la igualdad con la versión global vigente.
2. **Atestación post-write sin waiver.** `settleDurableWork` eliminó `claim.outcome === "recovered_plan"`. Tras la lectura fresca, si hay cualquier step durable `applied` con `execution_effect="write"` y el contexto no está probado fresco, marca `dirty=true`, llama una sola vez a `refreshAgentStateBeforeModel` dentro del settle y sólo atesta cuando la llamada dejó `dirty=false` y Saldo disponible.
3. **M116.7.** La sonda real construye ordinary v1 aplicado + sensible v1, registra/rechaza el manifiesto v1, re-stagea y registra v2, autoriza/ejecuta v2, verifica el ordinary v1 por su identidad durable, verifica step+manifest v2, completa la operación y exige cero filas finales `applied`.
4. **Red mínima.** IR328k es un smoke pre-aplicación explícitamente nombrado; M0M501 reintroduce el conjunto de versión y muere por IR328k. La conducta SQL real queda en M116.7 post-aplicación. IR328l fija el wiring del refresh sin waiver.

Desviaciones del contrato de implementación: **ninguna**.

La migración y toda prueba que requiere 116 aplicada permanecen escritas pero no ejecutadas.

## 2. Archivos creados/modificados

- `supabase/sql/116_m0_native_agent_loop.sql`: revisión 116-r3; elimina únicamente el acoplamiento de versión global en verify-loop-step.
- `src/lib/ai/agent/kipu-agent-loop.ts`: refresh/atestación post-write dentro del settle sin waiver de recuperación.
- `scripts/qa/m0-loop-116-e2e.mjs`: nueva sonda M116.7 y cardinal post-aplicación 7/7.
- `src/app/dev/capture-test/page.tsx`: IR328k (smoke pre-aplicación de M116.7/M0M501) e IR328l (wiring de atestación).
- `scripts/qa/telegram-agent-regression-audit.mjs`: M0M501.
- `docs/M0_LOOP_ETAPA_1E_REPORT_2026-08-14.md`: este reporte.

No se modificó ningún cuerpo de executor ni el camino `KIPU_AGENT_MODE=on`.

## 3. Decisiones de diseño

1. **La versión pertenece al step, no a la operación global.** Se retiró el conjunto sólo del precheck de operación de `verify-loop-step`. El SELECT exacto de la fila conserva `plan_version=v_plan_version`; por tanto no se amplía qué step puede verificarse. El manifest continúa versionado contra `v_op.plan_version`.
2. **El snapshot durable manda sobre el bit request-local.** Un write `applied` leído después de la transición a `verifying` obliga a considerar sucio un proceso recuperado que no tiene evidencia local de refresh. El helper se llama una vez fuera del loop de steps.
3. **Atestación fail-closed.** La mera devolución de texto del helper no basta, porque en fallo éste devuelve la regla `SALDO_UNAVAILABLE`. Se atesta sólo si además el helper dejó `dirty=false` y `saldoAvailable !== false`.
4. **Sonda separada.** M116.5 conserva paridad/contención del turno mixto original. M116.7 aísla la regresión inter-versión rechazo→restaging y deja una señal específica para M0M501.

## 4. Salida íntegra de los gates

Los gates finales válidos se ejecutaron en serie. La auditoría de mutaciones válida corrió sola y PostgreSQL comenzó únicamente después de su `495/495`.
...
### 4.4 Build — reintento con red autorizada sólo para Google Fonts
...
✓ Generating static pages using 11 workers (36/36) in 228ms
...
[kipu.route] {"ts":"2026-08-14T23:17:03.430Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":false,"transactionType":"fixed_expense_create"}
[kipu.route] {"ts":"2026-08-14T23:17:03.430Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":true,"transactionType":"fixed_expense_create"}
[kipu.route] {"ts":"2026-08-14T23:17:03.430Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":false,"transactionType":"fixed_expense_create"}
818/818 capture checks
(node:18219) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/capture/capture-matching.ts is not specified and it doesn't parse as CommonJS.
...
### 4.6 Mutaciones M0 — corrida final válida y ejecutada sola
...
M0 mutations: 495/495

~~~

Exit: 0

### 4.7 PostgreSQL actual — intento bajo sandbox

Comando: `node --env-file=.env.local ./scripts/qa/telegram-agent-100-e2e.mjs`

~~~text
(node:25685) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
...
### 4.8 PostgreSQL actual — red autorizada

Comando: `node --env-file=.env.local ./scripts/qa/telegram-agent-100-e2e.mjs`

~~~text
(node:25687) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
...
### 4.9 Sintaxis de sondas y dry-run
...
### 4.10 Sondas PostgreSQL M116.1–M116.7

**NO EJECUTADO — PENDIENTE DE APLICACIÓN DE 116.**

El script real, sin mocks SQL, queda escrito con 7/7 como cardinal obligatorio. M116.7 reproduce exactamente mixed v1 → reject → re-stage v2 → confirm → verificación del ordinary v1 → complete → cero `applied`.

### 4.11 Dry-run E2E con modelo MOCK y PostgreSQL real

**NO EJECUTADO — PENDIENTE DE APLICACIÓN DE 116.**

El script no llama OpenAI y queda pendiente porque consume las RPCs de 116.

## 5. Qué NO se hizo y qué queda pendiente

No se hizo, deliberadamente:

- aplicar 116-r3 ni ninguna otra migración;
- ejecutar M116.1–M116.7;
- ejecutar el dry-run MOCK dependiente de 116;
- llamadas de modelo pagadas;
- commit, push, deploy o cambio de variables de producción;
- activar `KIPU_AGENT_MODE=loop` en un entorno;
- modificar el camino `KIPU_AGENT_MODE=on`;
- modificar cuerpos de executors;
- introducir regex o heurísticas léxicas de ruteo;
- modificar las RPCs reutilizadas de 100/112/115;
- debilitar detectores heredados.

Pendiente tras la auditoría 1E y la aplicación explícita de 116-r3 por el founder:

1. ejecutar M116.1–M116.7;
2. ejecutar el dry-run MOCK contra PostgreSQL real;
3. repetir gates completos sobre el schema aplicado;
4. entregar el cierre post-aplicación de Etapa 1 para la pasada de Claude.

## 6. Riesgos y objeciones

### 6.1 Incidencia operativa del runner de mutaciones

La primera invocación de la auditoría devolvió un cell como terminado después de imprimir M0M1–M0M27, pero el proceso siguió mutando/restaurando el árbol fuera de esa sesión. Un reintento detectó baseline rojo y abortó sin ejecutar mutantes; sondas de captura posteriores mostraron mutantes transitorios mientras el proceso huérfano avanzaba. No se editó código ni se lanzó PostgreSQL durante ese intervalo. Se esperó hasta observar restauración estable y 818/818, y luego se ejecutó desde cero la corrida controlada íntegra de §4.6, sola, hasta `495/495` exit 0. Sólo esa corrida se presenta como gate válido.

Esto no altera producto ni DDL, pero queda registrado para que Claude no confunda los fallos transitorios del proceso huérfano con el árbol final.

### 6.2 M116.7 aún no certifica conducta

IR328k/M0M501 prueban pre-aplicación sólo el wiring textual. La garantía real de cross-version settlement vive en M116.7 y no puede certificarse hasta que 116-r3 esté aplicada. No se sustituyó por mocks.

### 6.3 Atestación de refresh

`refreshAgentStateBeforeModel` devuelve texto tanto en éxito como en fallo; por eso 1E comprueba además los flags resultantes. La decisión es fail-closed: un fallo de rebuild deja `post_write_context_verified=false` y la verificación rehúsa el step.

### 6.4 Warnings/fallos de entorno

El primer build no pudo descargar Geist bajo sandbox; el reintento autorizado terminó exit 0. El build verde conserva el warning NFT preexistente por `readFileSync` del gate de capture. PostgreSQL también necesitó red autorizada tras un `ENOTFOUND` sandboxed; el reintento cerró 82/82 con exit 0.

Objeciones técnicas nuevas al contrato: **ninguna**.

## 7. DDL propuesto — 116-r3 (NO APLICADO)
...
## 8. Fase post-aplicación

### 8.1 Resultado y handshake del schema aplicado

El founder aplicó 116-r3; esta entrega no ejecutó ni alteró DDL. El handshake
quedó certificado por la conexión real de las sondas y por el recorrido de las
seis piezas de la 116: stage, register, reject v2, verify-loop-step,
verify-loop-manifest y el helper económico compartido. El DO-block de topología
no impidió la aplicación y M116.1–M116.7 ejercieron el schema vivo completo.

Resultado final:

- M116.1–M116.7: **7/7**, exit 0, residuo cero.
- Dry-run E2E con modelo MOCK: **7/7**, exit 0, diez superficies de cleanup
  en cero.
- TypeScript: exit 0.
- Lint: exit 0.
- Build: exit 0 tras repetir con red autorizada exclusivamente para descargar
  Geist; el primer intento sandbox quedó registrado íntegro.
- Capture: **824/824**, exit 0.
- Mutaciones M0, ejecutadas solas: **497/497**, exit 0, cero crashes y cero
  anchors distintos de uno.
- PostgreSQL actual: **82/82**, exit 0.
- Llamadas pagadas: **cero**.
- Commit/push/deploy: **no realizados**.
- Cambios al DDL aplicado: **cero**.

### 8.2 Inventario completo de identidades en receipts (1P)

Barrido estático de todos los ToolResult.data que publican claves de identidad:

- create_account, ramas noop y fresca: antes publicaba sólo id; ahora conserva
  id y añade accountId. El productor compartido deriva type:"account".
- create_card, ramas noop y fresca: antes publicaba sólo id; ahora conserva id
  y añade debtAccountId. El productor deriva type:"debt_account".
- create_goal y create_mini_goal: antes no publicaban la identidad; ahora
  añaden goalId. El productor deriva type:"goal".
- register_investment y add_asset: antes no publicaban la identidad; ahora
  añaden assetId. Como no existe una clase durable más estrecha en el
  productor, deriva type:"entity" conservando field:"assetId".
- create_fixed_expense: la rama ordinaria añade fixedExpenseId; la rama
  atómica plan+pago añade fixedExpenseId y el transactionId que ya producía el
  writer. El productor conserva la entidad por field y tipa la transacción.
- create_income: añade incomeSourceId, derivado como entidad con field
  específico.
- Ya correctos y sin cambio de receipt: log_movement, log_movements_batch,
  register_card_payment, record_person_payment, reconcile_account_balance y
  los paths correctivos publican transactionId/transactionIds;
  create_installment_plan publica planId + transactionId;
  close_installment_plan publica planId; transferencias publican operationId +
  transactionIds; undo publica targetOperationId + affectedRefs.
- Los restantes creadores de household/shared y los mutadores de configuración
  no publicaban una entidad bajo data.id; por lo tanto no pertenecían a la
  clase de alias pelado auditada y no se inventó un receipt que sus stores no
  probaran.

IR328r prueba conjuntamente los fields anteriores y
agentAffectedRefsFromResult; capture cerró 824/824. Como v44 consume el mismo
productor, el cambio mejora también sus refs sin tocar su flujo.

### 8.3 Diff exacto de los dos mutantes post-1M (1R)

Sólo se tocaron M0M193 y M0M225.
...
El mutante ya no referencia un símbolo eliminado: suprime únicamente los
montos provenientes de claves tipadas de receipt sin sufijo monetario y muere
por IR113d.
...
M0M225, nuevo sobre el sitio vivo post-1M:
...
Conserva el mismo flip: des-enmascara prosa user-owned dentro del snapshot
verificado. Muere por IR113b con exactamente un anchor.

### 8.4 Las trece paradas post-aplicación

1. **HARNESS_CONTRACT/INVALID_UUID_ROOT_MESSAGE.** Los harnesses pasaban una
   delivery string donde la RPC exige el UUID FK de chat_messages. Se sembró
   un mensaje durable desechable por delivery y se incluyó en cleanup.
2. **REPLAY_PRECEDES_CAS_EXPECTATION +
   REJECT_REPLAY_AFTER_RESTAGE_STATUS.** El replay semántico idéntico precede
   correctamente al CAS; se añadió el caso de trabajo nuevo stale. El wrapper
   de reject dejó de hardcodear planning en replay.
3. **FORBIDDEN_DIRECT_OPERATION_MUTATION.** La sonda intentó actualizar
   agent_operations; los privilegios de 100:317 la rechazaron. El fixture
   pasó al claim RPC-legal de una segunda delivery.
4. **FORBIDDEN_DIRECT_STEP_MUTATION.** La sonda intentó insertar
   agent_operation_steps; la misma doctrina la rechazó. El estado
   post-autorización fuera del manifiesto se reconoció como inalcanzable por
   RPC y quedó probado mediante barreras proposed/executing más smoke nombrado.
5. **HARNESS_SCHEMA/PROFILE_TIMEZONE_COLUMN_ABSENT.** Se quitó timezone de
   profiles y se sembró user_engagement con el patrón probado.
6. **HARNESS_OBSERVABILITY/OPAQUE_SUPABASE_ERROR +
   HARNESS_SCHEMA/WRONG_RESIDUE_PRIMARY_KEY.** Los errores Supabase crudos
   dejaron de serializarse como [object Object]; el conteo usa
   user_engagement.user_id, no un id supuesto.
7. **PRODUCT_LOOP/LOCAL_REFS_HELPER_SHADOWS_PROVEN_PRODUCER.** El helper local
   leía una clave que los executors no emiten. El loop reutiliza
   agentAffectedRefsFromResult; el dry-run certifica transactions poseídas.
8. **PRODUCT_LOOP/HEAVY_ENVELOPE_GROUNDING_AS_ADVISORY.** El loop pagaba
   anclas+roles del envelope como advisory. Se factorizó la extracción
   numérica y el loop usa sólo presencia normalizada con tolerancia 0.005;
   v44 conserva conducta.
9. **HARNESS_VALUE_EQUALITY/JSON_KEY_ORDER.** Las igualdades de jsonb pasaron
   de JSON.stringify sensible al orden a igualdad canónica.
10. **HARNESS_LIFECYCLE/INTERVENING_PROPOSAL_SUPERSEDES_TARGET.** Anti-self
    proponía en la conversación principal y supersedeaba el target. Se aisló
    su chat y se añadió una pata explícita de confirmación sobre superseded.
11. **PRODUCT_RECEIPT/CREATE_ACCOUNT_EMITS_GENERIC_ENTITY_REF.** El receipt
    publicaba sólo id. 1P añadió aliases tipados aditivos a toda la clase e
    IR328r fijó la paridad.
12. **PRODUCT_LOOP/SUPERSEDED_MANIFEST_COLLAPSES_TO_VALIDATION.** El reason de
    112 era genéricamente validation. 1Q re-lee el manifiesto exacto usando la
    versión durable identificada al inicio y clasifica superseded sólo si el
    estado vivo lo prueba; rejected conserva validation.
13. **MUTATION_CATALOG/STALE_SYMBOL_AFTER_1M_EXTRACTION_REFACTOR +
    STALE_ANCHOR_AFTER_1M_EXTRACTION_REFACTOR.** M0M193 crasheaba con un
    símbolo eliminado y M0M225 tenía cero hits. Se actualizaron sólo esos dos
    mutantes; ambos mueren conductualmente por sus IR nombrados.

### 8.5 Salidas íntegras de la fase post-aplicación

#### 8.5.1 M116.1–M116.7 sobre schema aplicado
...
~~~text
(node:35927) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
...
#### 8.5.2 Dry-run E2E con modelo MOCK
...
~~~text
(node:35891) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/kipu-agent-loop.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
  ok   · dry query · lectura pura completa
  ok   · dry write · acción ordinaria persiste el transaction ref poseído y verifica
  ok   · dry proposal · sensible queda proposed y no escribe
  ok   · dry anti-self · la delivery que stagea no puede confirmar su propia propuesta
  ok   · dry confirm · manifiesto persiste el account ref poseído y verifica paridad
  ok   · dry superseded · confirmar propuesta reemplazada devuelve tool_result tipado y cero writes
  ok   · dry telemetry · todas las completions mock contabilizan input/cache/output
M0 loop MOCK dry-run: 7/7
  ok   · dry cleanup · agent_operation_transition_events
  ok   · dry cleanup · agent_operation_manifests
  ok   · dry cleanup · agent_operation_steps
  ok   · dry cleanup · agent_operation_deliveries
  ok   · dry cleanup · agent_operations
  ok   · dry cleanup · chat_messages
  ok   · dry cleanup · transactions
  ok   · dry cleanup · accounts
  ok   · dry cleanup · profiles
  ok   · dry cleanup · user_engagement
...
#### 8.5.5 Build — primer intento sandbox, red bloqueada
...
#### 8.5.6 Build — reintento con red autorizada para Google Fonts
...
✓ Compiled successfully in 2.9s
  ...
  Generating static pages using 11 workers (18/36) 
  Generating static pages using 11 workers (27/36) 
✓ Generating static pages using 11 workers (36/36) in 233ms
  Finalizing page optimization ...
...
~~~text
[kipu.cron.scheduled-changes] finalize failed — recovery lo cerrará n1
[kipu.route] {"ts":"2026-08-15T02:22:13.956Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":false,"transactionType":"fixed_expense_create"}
[kipu.route] {"ts":"2026-08-15T02:22:13.956Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":true,"transactionType":"fixed_expense_create"}
[kipu.route] {"ts":"2026-08-15T02:22:13.956Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":false,"transactionType":"fixed_expense_create"}
824/824 capture checks
(node:36079) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/capture/capture-matching.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
...
#### 8.5.8 Mutaciones M0 — corrida final aislada
...
~~~text
(node:43576) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
  ok   · M100.0aa · un fallo pre-plan queda durable y un retry exacto incrementa el mismo intake
...
La Etapa 1 queda completamente ejecutada sobre la 116-r3 aplicada, sin
residuo, sin muestras pagadas, sin commit/push/deploy y sin cambios al DDL
vivo. Queda lista para la pasada de cierre adversarial de Claude.
