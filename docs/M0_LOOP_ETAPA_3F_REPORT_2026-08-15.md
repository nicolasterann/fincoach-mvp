...
Fecha: 2026-08-15  
Estado: implementación y gates completos terminados; **DETENIDA antes de cualquier corrida pagada**, pendiente de auditoría de Claude.  
Árbol: worktree local no comiteado; sin commit, push, deploy ni cambios/aplicación de DDL.  
Producción: `KIPU_AGENT_MODE=on` intacto; no se ejecutaron writes contra la cuenta real del founder.

## 1. Alcance implementado frente al contrato 1X

1. **Ownership del receipt en borrowed.** El único return de éxito de la rama `direction="in"` + `inflowKind="borrowed"` de `record_person_payment` declara `operationStepReceipt: "writer"`. Ese return común cubre tanto fresh (`effect:"wrote"`) como replay (`effect:"noop"`).
2. **Fallos conservan ownership genérico.** El return `!applied.ok` no declara `operationStepReceipt`; `needs_info` y cualquier fallo anterior tampoco lo declaran. El SQL hizo rollback en esas ramas y el step conserva el lifecycle genérico.
3. **Cero cambios fuera del alcance de producto.** No se tocó `kipu-agent-loop.ts`, el orquestador `kipu-agent.ts`, el writer SQL, otro executor ni ningún DDL.
4. **IR334 de clase.** El check deriva de todos los SQL del repo el conjunto de funciones loop que actualizan directamente `agent_operation_steps.status='applied'`, excluye explícitamente `kipu_apply_operation` porque pertenece al lifecycle envelope, y exige igualdad exacta con `kipu_apply_debt_proceeds` + `kipu_reverse_agent_operation`. Después exige una sola declaración `writer` en el return exitoso de cada executor y cero en el fallo borrowed. Un tercer auto-writer SQL sin espejo rompe el check.
5. **Mutantes.** M0M521 elimina la declaración borrowed y muere por IR334. El mutante histórico M0M450 fue acotado al return de undo y ahora también declara IR334 como su detector de clase; M0M449 conserva IR302 para la barrera del orquestador.

## 2. Archivos modificados en 1X

- `src/lib/ai/agent/kipu-agent-tools.ts`: una única declaración de producto `operationStepReceipt: "writer"` en el return borrowed exitoso.
- `src/app/dev/capture-test/page.tsx`: IR334, inventario SQL↔executor y pin de las ramas éxito/fallo.
- `scripts/qa/telegram-agent-regression-audit.mjs`: M0M521; acotación de M0M450 y reasignación de su detector esperado a IR334.
- `docs/M0_LOOP_ETAPA_3F_REPORT_2026-08-15.md`: este reporte.

## 3. Contrato de frontera verificado

### 3.1 Fresh y replay comparten la declaración correcta

El executor recibe de `kipu_apply_debt_proceeds` tanto el resultado fresh como el idempotente y retorna por el mismo bloque. Por eso una declaración en ese return cubre exactamente ambos casos exitosos, sin introducir una bifurcación ni lógica nueva.

### 3.2 Fallo y needs_info no reclaman un receipt que el writer no asentó

La declaración aparece después de `if (!applied.ok)` y después de `ctx.dirty=true`/refresh. IR334 comprueba esa posición: el slice del return fallido contiene cero declaraciones y el slice exitoso contiene exactamente una. El loop conserva así el recorder genérico cuando no hubo commit del writer.

### 3.3 Inventario definitivo de auto-settlers

El barrido sobre las funciones SQL vigentes encontró exactamente dos writers loop que asientan su propio step:

- `public.kipu_reverse_agent_operation` → `undo_agent_operation` → declaración `writer` ya existente.
- `public.kipu_apply_debt_proceeds` → rama borrowed de `record_person_payment` → declaración `writer` añadida por 1X.

`public.kipu_apply_operation` se excluye de manera nombrada porque es el applier del envelope legacy y el loop no lo usa. La igualdad de conjuntos hace fallar IR334 si aparece un tercer auto-settler.

## 4. Incidentes de QA durante la red

### 4.1 `M0M450_ANCHOR_CARDINALITY_AFTER_SECOND_WRITER`

La primera auditoría aislada llegó a M0M521, que murió correctamente, pero M0M450 falló antes de mutar porque su anchor era la cadena global `operationStepReceipt: "writer"` con cardinal esperado 1; 1X hace que existan deliberadamente dos. Se acotó el `from/to` de M0M450 al bloque `effect: reversed.replayed ... summary: reversed.replayed`, manteniendo su flip conductual.

Salida exacta del incidente:

~~~text
FAIL · M0M450 whole-operation undo stops declaring that its database transaction owns the durable step receipt: anchor hits=2, expected 1
ok · M0M521 borrowed debt proceeds lose writer-owned operation-step receipt → IR334
M0 mutations: FAIL
~~~

### 4.2 `M0M450_DETECTOR_MOVED_TO_CLASS_PIN`

Con el anchor acotado, el mutante de undo fue correctamente matado por IR334 antes que por su detector histórico IR302. El runner exige que el detector que muerde coincida con el declarado, por lo que la segunda auditoría quedó roja. La resolución conserva IR302 en M0M449 (orquestador no duplica receipts) y asigna IR334 a M0M450 (paridad auto-writer SQL↔executor).

Salida exacta del incidente:

~~~text
ok · M0M449 the orchestrator persists a second receipt after a writer already settled its own step → IR302
FAIL · M0M450 whole-operation undo stops declaring that its database transaction owns the durable step receipt → IR302
850/851 capture checks
✗ IR334 · cada writer SQL loop que auto-asienta su step declara ownership exactamente en su return de éxito — {"ir334SqlReceiptWriters":["kipu_apply_debt_proceeds","kipu_reverse_agent_operation"],"undoDeclarations":0,"borrowedDeclarations":1,"borrowedFailureOwnsReceipt":false}
ok · M0M521 borrowed debt proceeds lose writer-owned operation-step receipt → IR334
M0 mutations: FAIL
~~~

Ambos incidentes fueron del catálogo de mutaciones al adquirir la nueva cobertura de clase. No hubo cambio adicional de producto. La corrida final completa quedó 515/515.

## 5. Salidas íntegras de gates

Orden contractual ejecutado: M117 → dry-run MOCK → tsc → lint → capture → build → mutaciones solas → PostgreSQL. La cola `tsc → lint → capture → build → mutaciones` se repitió sobre el catálogo final después de corregir los anchors. No hubo ningún proceso de QA paralelo a mutaciones.

### 5.1 M117 post-aplicación

Comando: `node --env-file=.env.local ./scripts/qa/m0-loop-117-e2e.mjs`

~~~text
(node:47827) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
...
### 5.2 Dry-run MOCK loop — 14 patas
...
loopUsage agregado: {"cachedInputTokens":2000,"calls":50,"inputTokens":5000,"outputTokens":1000}
...
Costo real acumulado: 0.025016 USD
...
Exit: 0. Resultado: 14/14; `DRY_BORROWED_LINK` cruza proposal → confirm → write caja+deuda → step verified → manifest verified → respuesta con receipts; residuo cero. Modelo MOCK: gasto facturado USD 0. El `0.025016 USD` impreso es valoración sintética de la telemetría MOCK.
...
[kipu.route] {"ts":"2026-08-16T02:40:52.936Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":false,"transactionType":"fixed_expense_create"}
[kipu.route] {"ts":"2026-08-16T02:40:52.937Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":true,"transactionType":"fixed_expense_create"}
[kipu.route] {"ts":"2026-08-16T02:40:52.937Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":false,"transactionType":"fixed_expense_create"}
851/851 capture checks
(node:56287) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/capture/capture-matching.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
~~~

Exit: 0. Resultado: 851/851.
...
### 5.7 Mutaciones — corrida final aislada
...
M0 mutations: 515/515
~~~

Exit: 0. Resultado: 515/515. M0M450 → IR334; M0M521 → IR334. La auditoría corrió sola.

### 5.8 PostgreSQL completo

Comando: `node --env-file=.env.local ./scripts/qa/telegram-agent-100-e2e.mjs`

~~~text
(node:60186) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
...
1. **Ninguna objeción bloqueante al contrato 1X.** La declaración es una propiedad del receipt, no un branch nuevo ni autoridad del modelo.
2. **Worktree acumulado.** El árbol sigue conteniendo el trabajo no comiteado de etapas anteriores; la lista `git diff` global incluye esos cambios. Esta ronda sólo añadió el delta enumerado en §2 y no tocó el orquestador ni DDL.
3. **Warning existente de build.** Next/Turbopack informa que el import server-side del capture test provoca tracing amplio. No es nuevo de 1X y el build termina exit 0.
4. **Warnings de módulo.** Node avisa que algunos `.ts` se reparsean como ESM por falta de `type:module`. No afectan los exits ni se corrigieron fuera de alcance.

## 7. DDL y operaciones externas

- DDL creado o modificado en 1X: **ninguno**.
- Migraciones aplicadas por Codex: **ninguna**.
- Commit/push/deploy: **ninguno**.
- Writes contra cuenta real del founder: **ninguno**.
- Llamadas pagadas: **ninguna**.
- Corrida final 2: **NO EJECUTADA**; queda detenida hasta el OK de Claude.

## 8. Veredicto solicitado

El contrato 1X está implementado y verificado. La frontera SQL auto-settling writer ↔ executor receipt queda fijada por igualdad de conjunto, los dos mutantes de writer mueren por IR334 y todas las baterías contractuales están verdes. La siguiente acción permitida es únicamente la auditoría de Claude; no se lanzó ninguna corrida pagada.
