...
Fecha: 2026-08-15  
Estado: **LISTA PARA AUDITORÍA DE CLAUDE**  
Muestras pagadas: **un smoke autorizado; 3 escenarios de agente iniciados, 2 jueces completados; cero corridas completas**  
Migraciones aplicadas por Codex: **0**  
DDL modificado o aplicado: **no**  
Commit / push / deploy: **no realizados**

## 1. Alcance implementado vs. contrato de la etapa

Se leyó completo `docs/M0_CLAUDE_AUDIT_TOOL_LOOP_2026-08-14.md`, incluidos §6, §7 Etapa 2, §8 y ADENDA 5, antes de construir esta etapa. También se leyó completa la guía local de Next 16.2.4 `node_modules/next/dist/docs/01-app/01-getting-started/15-route-handlers.md` antes de cerrar el cambio de la ruta de evaluación.

Alcance entregado:

1. **Runner black-box parametrizado.** `scripts/qa/m0-loop-conversation-e2e.mjs` exige `--mode=loop|on`, cruza el transporte HTTP local autorizado y verifica PostgreSQL. No importa el loop ni el planner. El handshake rehúsa medir si el modo vivo no coincide.
2. **Carril DINERO, duro.** Cada escenario declara invariantes de estado: filas y tipos de ledger, montos exactos, saldos locales, reversas append-only por `related_transaction_id`, receivables, deudas, cuentas y paridad durable de manifiestos por valor canónico.
3. **Carril CONDUCTA, duro.** Rehúsa HTTP no-2xx, respuesta vacía, fuga de wire por el regex transversal autorizado, repetición de la misma pregunta normalizada sin cambio de progreso durable y cualquier write sensible sin manifiesto autorizado previo. No hay aserciones de frases de respuesta.
4. **Carril CALIDAD, graduado.** Juez mini a temperatura 0 con cuatro dimensiones enteras 1–5: resolución, preguntas necesarias, números correctos y voz humana. Imprime JSON y promedio por escenario; nunca convierte copy léxico en gate duro.
5. **Catálogo completo.** Declara los 24 escenarios actuales, el transcript real del arriendo, el de los cuatro créditos y ocho familias aspiracionales con tres paráfrasis cada una: **50 escenarios**. Los escenarios heredados conservan las aserciones monetarias valiosas de ME6, ME10aa, ME11–ME14, ME16 y ME17.
6. **Paráfrasis runtime.** La corrida completa usa el modelo mini para producir paráfrasis nuevas preservando semántica y números; dry-run y smoke usan las variantes enlatadas para no añadir gasto no autorizado.
7. **Dry-run MOCK.** Inyecta el modelo exclusivamente a través del handler público local, no por imports privados, y recorre read-only, write ordinario y propuesta+confirmación sensible contra PostgreSQL real. Valida además que los 50 contratos tengan executor, seed y presupuesto de turnos.
8. **Smoke real único.** Se consumió el presupuesto en tres escenarios de agente. ME2 alcanzó el agente pero el intento inicial de juez con `gpt-5-mini` fue rechazado porque ese modelo no admite `temperature:0`; se detuvo sin ejecutar los otros dos. Se cambió sólo el juez a `gpt-4.1-mini` y se reanudaron únicamente `ASP_PURCHASE_DECISION_1` y `ASP_HUMAN_COACHING_1`: 2/2 duros verdes, calidad 4.75/5, residuo cero. No se repitió ME2 ni hubo otra muestra pagada.
9. **Telemetría y costo.** Agrega `loopUsage`, judge usage y paraphrase usage. Proyecta 107 turnos/50 escenarios. El smoke observado proyectó USD 2.80 para la corrida completa; el árbol final añade una reserva determinista para la única llamada de paráfrasis que el smoke no ejecuta. El dry-run final la muestra explícitamente.
10. **Reglas heredadas del embudo.** Seed de `chat_messages` raíz, `profiles` sin timezone, `user_engagement.timezone`, cuenta `is_currency_default`; JSONB canónico; errores no-Error serializados como `{code,message,details,hint}`; residue count por PK real; cero writes directos a tablas protegidas; personas marcadas y 24 superficies de cleanup por identidad.
11. **Red determinista.** IR329 fija separación de carriles/black-box/cero frases; IR329b fija las reglas heredadas; IR329c fija dry read/write/sensitive, telemetría y costo completo. M0M35 y M0M212 se reanclaron al puente vivo sin cambiar su conducta: ambos mueren por TG-12.
12. **Baseline registrado.** `--mode=on` se imprime como **HÍBRIDO v44+M0.11A**.

Desviaciones del contrato: **ninguna funcional**. La corrida real completa de 50 escenarios y el head-to-head `loop|on` no pertenecen al presupuesto de Etapa 2 y se dejaron para Etapa 3. El dry-run prueba plomería representativa y contratos estáticos; no pretende certificar semántica de los 50 escenarios con un guion MOCK.

## 2. Archivos creados/modificados

- `scripts/qa/m0-loop-conversation-e2e.mjs`: runner black-box, catálogo de 50 escenarios, tres carriles, dry-run MOCK, smoke, cleanup, telemetría y estimación.
- `src/app/dev/m0-agent-eval/route.ts`: handshake de modo e inyector guionado local/autorizado para `loop`; la ruta sigue cerrada en producción, despliegues Vercel, hosts no-loopback y sin bearer.
- `src/lib/ai/chat-transaction-handler.ts`: dependencia opcional `loopModel` para QA; todos los callers productivos la omiten y conservan el proveedor normal.
- `src/app/dev/capture-test/page.tsx`: IR329, IR329b e IR329c; ajuste de dos smokes de wiring preexistentes a la llamada multilínea.
- `scripts/qa/telegram-agent-regression-audit.mjs`: reubicación exacta de M0M35 y M0M212 al texto vivo del bridge.
- `scripts/qa/README.md`: comandos, modos, catálogo, tres carriles y baseline híbrido.
- `docs/M0_LOOP_ETAPA_2_REPORT_2026-08-15.md`: este reporte.

No se modificó ningún executor, ningún SQL ni el comportamiento productivo de `KIPU_AGENT_MODE=on`.

## 3. Decisiones de diseño

1. **Inyección en el límite público.** El mock entra en el handler completo únicamente desde la ruta QA local. Así el dry-run atraviesa persistencia del mensaje raíz, identidad de delivery, contexto, loop, store y PostgreSQL sin convertir un import privado en prueba de producto.
2. **Tres recorridos MOCK, cincuenta contratos estáticos.** Un guion determinista no razona sobre 50 conversaciones y fingir esa cobertura sería garantía falsa. Los tres recorridos prueban las tres topologías; la semántica completa queda para el modelo real en Etapa 3.
3. **`gpt-4.1-mini` como juez.** Es modelo mini y admite el requisito innegociable `temperature:0`. `gpt-5-mini` rechazó ese valor; no se relajó la temperatura para acomodarlo.
4. **Carriles independientes.** Calidad baja no maquilla un rojo monetario o estructural. DINERO y CONDUCTA determinan el exit; CALIDAD se imprime y agrega.
5. **Sensibilidad espejada mecánicamente.** El harness black-box replica las 32 capabilities siempre sensibles y ocho reglas condicionales, sin heurística sobre el mensaje. Esto permite auditar por SQL que cada write sensible tuvo autorización previa.
6. **Comparación canónica universal.** Toda igualdad de objetos provenientes de JSONB usa orden recursivo o deep equality; nunca depende del orden textual de claves.
7. **Cleanup por borrado de persona + inspección.** La persona lleva `runTag`; el borrado auth hace el cleanup normal y luego se cuentan 24 superficies por su PK real. Finalmente se recorre el catálogo auth para detectar huérfanos marcados.
8. **Costo observado + reserva.** La proyección escala la telemetría real/MOCK por 107 turnos y 50 juicios. Si el smoke no ejecutó generación de paráfrasis, reserva una llamada mini mediante una estimación determinista por tamaño de payload, en vez de imprimir falsamente cero.
9. **Cero frase-gates.** El único regex sobre salida es el anti-fuga exigido. La repetición de pregunta se decide por normalización + progreso durable, no por una frase esperada.

## 4. Salida íntegra de los gates

Todos los gates definitivos se ejecutaron en serie. Las mutaciones corrieron solas, con servidor local apagado y sin ningún otro gate en paralelo.

### 4.1 Sintaxis del runner

Comando: `node --check scripts/qa/m0-loop-conversation-e2e.mjs`
...
### 4.4 Build — primer intento bajo sandbox
...
Exit: 1. Causa: el sandbox no pudo descargar Geist/Geist Mono.

### 4.5 Build final con red autorizada
...
✓ Generating static pages using 11 workers (36/36) in 203ms
...
Comando: `curl -sS http://127.0.0.1:3000/dev/capture-test`

~~~text
827/827 aserciones pasan
~~~

HTTP/exit: 200/0

### 4.7 Dry-run MOCK final
...
Dry-run MOCK: ejecuta 3 recorridos representativos y valida estáticamente los 50 contratos del catálogo.
...
loopUsage agregado: {"cachedInputTokens":280,"calls":7,"inputTokens":700,"outputTokens":140}
Judge usage agregado: {"cachedInputTokens":0,"calls":3,"inputTokens":540,"outputTokens":135}
Paraphrase usage agregado: {"cachedInputTokens":0,"calls":0,"inputTokens":0,"outputTokens":0}
Calidad promedio: 5.00/5
Costo estimado corrida completa: {"baseline":"native loop","basis":"MOCK token telemetry plus a deterministic paraphrase allowance, scaled to the complete catalog","estimatedTurns":107,"estimatedUsd":0.09,"ratesUsdPerMillion":{"coach":{"cached":0.25,"input":2.5,"output":15},"mini":{"cached":0.1,"input":0.4,"output":1.6}},"scenarios":50,"tokens":{"agent":{"cachedInputTokens":7490,"calls":187,"inputTokens":18725,"outputTokens":3745},"judge":{"cachedInputTokens":0,"calls":50,"inputTokens":9000,"outputTokens":2250},"paraphrase":{"cachedInputTokens":0,"calls":1,"inputTokens":677,"outputTokens":736}}}
M0 tres carriles (loop, MOCK): 3/3 duros verdes

~~~

Exit: 0

### 4.8 Smoke real autorizado

Primer intento: `node --env-file=.env.local ./scripts/qa/m0-loop-conversation-e2e.mjs --mode=loop --smoke`

El runner completó el turno de agente de ME2 y se detuvo al primer fallo del juez antes de iniciar los otros escenarios. La causa exacta devuelta por OpenAI fue:

~~~text
400 Unsupported value: 'temperature' does not support 0 with this model. Only default (1).
~~~

Causa tipada: `JUDGE_MODEL_CONTRACT/TEMPERATURE_ZERO_UNSUPPORTED`. No se cambió el requisito; se eligió `gpt-4.1-mini`.

Reanudación acotada, sin repetir ME2:

`node --env-file=.env.local ./scripts/qa/m0-loop-conversation-e2e.mjs --mode=loop --smoke --scenario=ASP_PURCHASE_DECISION_1,ASP_HUMAN_COACHING_1`
...
[ASP_PURCHASE_DECISION_1] decidir una compra contra Saldo y compromisos · paráfrasis 1
  DINERO   PASS
  CONDUCTA PASS
  CALIDAD  {"average":5,"correct_numbers":5,"human_coach_voice":5,"model":"gpt-4.1-mini","only_necessary_questions":5,"rationale":"El coach respondió claramente que no conviene comprar las entradas por 120 USD dado el saldo actual y compromisos, usando números correctos y sin inventar. No hizo preguntas innecesarias y mantuvo un tono humano, claro y directo, sin sermonear.","resolved_request":5}
...
[ASP_HUMAN_COACHING_1] retomar y recibir una próxima acción humana · paráfrasis 1
  DINERO   PASS
  CONDUCTA PASS
  CALIDAD  {"average":4.5,"correct_numbers":5,"human_coach_voice":4,"model":"gpt-4.1-mini","only_necessary_questions":5,"rationale":"El coach retomó la situación financiera del usuario de forma clara y sin regaños, cumpliendo con la solicitud. Hizo solo una pregunta necesaria para avanzar, usó números correctos y actuales, y mantuvo un tono humano y claro, aunque podría ser un poco más cálido para evitar cualquier sensación de sermón.","resolved_request":4}
[ASP_HUMAN_COACHING_1] cleanup por identidad: cero
Residuo de personas por catálogo auth: cero

loopUsage agregado: {"cachedInputTokens":88576,"calls":4,"inputTokens":98633,"outputTokens":315}
Judge usage agregado: {"cachedInputTokens":0,"calls":2,"inputTokens":665,"outputTokens":208}
Paraphrase usage agregado: {"cachedInputTokens":0,"calls":0,"inputTokens":0,"outputTokens":0}
Calidad promedio: 4.75/5
Costo estimado corrida completa: {"baseline":"native loop","basis":"observed smoke/full telemetry scaled to the complete catalog","estimatedTurns":107,"estimatedUsd":2.8,"ratesUsdPerMillion":{"coach":{"cached":0.25,"input":2.5,"output":15},"mini":{"cached":0.1,"input":0.4,"output":1.6}},"scenarios":50,"tokens":{"agent":{"cachedInputTokens":4738816,"calls":214,"inputTokens":5276866,"outputTokens":16853},"judge":{"cachedInputTokens":0,"calls":50,"inputTokens":16625,"outputTokens":5200},"paraphrase":{"cachedInputTokens":0,"calls":0,"inputTokens":0,"outputTokens":0}}}
M0 tres carriles (loop, smoke real): 2/2 duros verdes
~~~

Exit: 0

Nota de estimación: esta salida pertenece a la corrida pagada anterior al último ajuste puramente determinista de la reserva de paráfrasis; por eso su JSON observado muestra `paraphrase.calls=0`. El dry-run final de §4.7 prueba el árbol entregado con `paraphrase.calls=1` reservado. No se reejecutó el smoke pagado para cambiar una línea de reporte de costo.

### 4.9 Mutaciones — incidente de anclas y corrida final íntegra

La primera corrida sobre Etapa 2 encontró exactamente:

~~~text
FAIL · M0M35 model E2E bypasses the public chat handler: anchor hits=0, expected 1
FAIL · M0M212 a successful model-eval turn omits the compiled runtime contract: anchor hits=0, expected 1
M0 mutations: FAIL
~~~

Ambas anclas apuntaban al formato previo de la ruta. Se actualizaron sólo sus `from/to`; los detectores TG-12 no se debilitaron.

Comando final, ejecutado solo: `node scripts/qa/telegram-agent-regression-audit.mjs`
...
M0 mutations: 497/497

~~~

Exit: 0

### 4.10 PostgreSQL — intento bloqueado por sandbox

Comando: `node --env-file=.env.local ./scripts/qa/telegram-agent-100-e2e.mjs`

~~~text
(node:56718) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
...
Exit: 1. Causa: DNS del sandbox, 0/0 antes de crear fixture.

### 4.11 PostgreSQL final con red autorizada

Comando: `node --env-file=.env.local ./scripts/qa/telegram-agent-100-e2e.mjs`

~~~text
(node:60544) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
...
Exit: 0 en la prevalidación; se repite después de escribir este reporte.

## 5. Qué NO se hizo a propósito y qué queda pendiente

1. No se ejecutó la batería real completa de 50 escenarios: el presupuesto de esta etapa autorizaba únicamente un smoke de 2–3 escenarios. La corrida completa y el head-to-head pertenecen a Etapa 3.
2. No se ejecutó `--mode=on`. El runner lo soporta y lo etiqueta **HÍBRIDO v44+M0.11A**; medirlo aquí habría sido una muestra pagada fuera de alcance.
3. No se presentó el dry-run como cobertura semántica de los 50 escenarios: son tres topologías reales + validación estática de catálogo.
4. No se relajó `temperature:0`; se cambió a un modelo mini que sí honra el contrato.
5. No se importó el loop/planner en el runner ni se hizo routing por regex del mensaje.
6. No se modificaron executors, SQL, DDL, migraciones ni tablas protegidas por DML directo.
7. No se hizo commit, push, deploy ni aplicación de migraciones.
8. No hubo más muestras pagadas que el único smoke autorizado.

Pendiente para Etapa 3, sujeto a autorización: ejecución completa `loop` y baseline `on`, comparación de DINERO/CONDUCTA/CALIDAD, costo real y cierre de diferencias.

## 6. Riesgos y objeciones

1. **Fuente literal del brief aspiracional no está en el árbol.** La búsqueda del repo encuentra el contrato “8 ejemplos/familias aspiracionales”, pero no un artefacto con ocho prompts canónicos. Se materializaron ocho familias coherentes con la superficie de producto: captura diaria, decisión de compra, decisión de tarjeta, gasto desconocido, meta, deuda, plan mensual y coaching humano. Claude debe confirmar que ese inventario representa el brief externo esperado; no se sustituyó por routing de frases.
2. **Mirror de sensibilidad.** El runner necesita ser black-box y por eso no importa el catálogo productivo; replica mecánicamente 32 capabilities + ocho reglas. IR329 fija la forma actual, pero un cambio futuro del catálogo exige actualizar este espejo o podría producir un falso negativo del gate de autorización.
3. **Calidad real todavía poco muestreada.** El smoke válido puntúa dos escenarios (4.75/5). ME2 consumió agente pero no juez por incompatibilidad del modelo inicial. No se gastó otra muestra para completar esa observación, preservando el límite autorizado.
4. **Estimación, no factura.** USD 2.80 escala sólo dos escenarios reales y ahora suma una reserva determinista de paráfrasis; el mix real de cache, writes y turnos largos —especialmente ME10a2— puede variar. El runner imprime tokens reales cuando se ejecute la batería completa.
5. **Warning NFT del build.** Next informa trazado amplio desde `next.config.ts` hacia el gate de capture. El build termina verde; el warning es preexistente y no se cambió configuración para ocultarlo.
6. **Ruido de módulo en PostgreSQL.** Node avisa que `agent-operation-store.ts` se reparsea como ESM porque `package.json` no declara `type:module`. La batería termina 82/82; cambiar el packaging sería una expansión fuera de etapa.
7. **El smoke no se reejecutó tras la reserva de costo.** El último delta sólo altera cálculo local de estimación y su IR; dry-run, capture, mutaciones y gates completos lo certifican. Repetir modelo real para cambiar `paraphrase.calls=0` a una reserva habría violado la disciplina de gasto.

## 7. DDL propuesto

**Ninguno.** No se modificó ni aplicó DDL en Etapa 2.


# APÉNDICE 1S — corrección post-auditoría de Etapa 2

Fecha: 2026-08-15  
Alcance: **sólo harness + IR; cero producto, cero DDL**

## 1S.1 Cambios

1. IR329d extrae por sintaxis el `ALWAYS_SENSITIVE` del runner y exige igualdad de conjunto contra `manifestAuthorizationPolicyForPlanner().always_requires_second_delivery`: **32=32**, sin faltantes ni sobrantes. También extrae `CONDITIONAL_SENSITIVITY_RULE_CODES` y exige igualdad contra los códigos productivos: **8=8**.
2. `evaluationFetch` convierte cualquier caída del transporte en `EvalServerUnreachableError` con causa `EVAL_SERVER_UNREACHABLE` y el comando `KIPU_AGENT_MODE=<modo> npm run dev`. Un fallo dentro de un escenario se re-lanza después de `finally`, por lo que limpia la persona actual y aborta la corrida en vez de gastar los escenarios restantes.
3. IR329e fija clase, code, comando, tres call sites y la eliminación de la degradación previa a `HTTP network`.
4. Al iniciar el head-to-head se detectó que el agregador leía sólo el shape de tokens del loop. El adaptador ahora consume también `promptTokens`, `cachedPromptTokens` y `completionTokens` del planner híbrido; IR329f fija por separado ambos contratos para que el costo de un modo no pueda volver a colapsar silenciosamente a cero.

## 1S.2 Sonda conductual de transporte

Comando:

`KIPU_MODEL_EVAL_URL=http://127.0.0.1:1 node --env-file=.env.local ./scripts/qa/m0-loop-conversation-e2e.mjs --mode=loop --dry-run`

~~~text
RUN_ABORT · {"message":"EVAL_SERVER_UNREACHABLE · no se pudo conectar con http://127.0.0.1:1. Inícialo con: KIPU_AGENT_MODE=loop npm run dev","stack":"EvalServerUnreachableError: EVAL_SERVER_UNREACHABLE · no se pudo conectar con http://127.0.0.1:1. Inícialo con: KIPU_AGENT_MODE=loop npm run dev"}

loopUsage agregado: {"cachedInputTokens":0,"calls":0,"inputTokens":0,"outputTokens":0}
Judge usage agregado: {"cachedInputTokens":0,"calls":0,"inputTokens":0,"outputTokens":0}
Paraphrase usage agregado: {"cachedInputTokens":0,"calls":0,"inputTokens":0,"outputTokens":0}
Calidad promedio: n/a/5
Costo estimado corrida completa: {"baseline":"native loop","basis":"MOCK token telemetry plus a deterministic paraphrase allowance, scaled to the complete catalog","estimatedTurns":107,"estimatedUsd":0,"ratesUsdPerMillion":{"coach":{"cached":0.25,"input":2.5,"output":15},"mini":{"cached":0.1,"input":0.4,"output":1.6}},"scenarios":50,"tokens":{"agent":{"cachedInputTokens":0,"calls":0,"inputTokens":0,"outputTokens":0},"judge":{"cachedInputTokens":0,"calls":50,"inputTokens":0,"outputTokens":0},"paraphrase":{"cachedInputTokens":0,"calls":1,"inputTokens":677,"outputTokens":736}}}
FAILURES: RUN_ABORT
M0 tres carriles (loop, MOCK): 0/3 duros verdes

~~~

Exit: 1 esperado; causa tipada presente y cero escenarios ejecutados.
...
Mutaciones, ejecutadas solas:

~~~text
M0 mutations: 497/497
~~~

Exit: 0. También: `node --check`, `npx tsc --noEmit` y `npm run lint` exit 0.

## 1S.4 Restricciones

No se modificó producto, SQL, DDL, juez, carriles, escenarios ni umbrales. No hubo llamadas pagadas, commit, push ni deploy.
