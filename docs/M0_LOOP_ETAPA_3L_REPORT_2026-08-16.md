# M0 loop — Reporte Etapa 3L / contrato 1AD

Fecha: 2026-08-16
Estado: implementación y gates completos; **sin commit, push, deploy, DDL ni llamadas pagadas**.

## 1. Alcance implementado frente al contrato 1AD

1. **Orden quirúrgico del confirm:** en la ejecución de un manifiesto recién autorizado, el `appendToolResult` de `confirm_operation` ocurre antes de `pushFreshAgentStateBeforeModel`. El historial ya no puede intercalar un `system` entre `assistant.tool_calls` y su respuesta `tool`.
2. **Invariante local pre-provider:** `loopMessagesSequenceIssue` y `loopMessagesSequenceValid` validan que todo mensaje assistant con tool calls sea seguido inmediatamente por exactamente una respuesta tool por id, sin otro rol intercalado, sin duplicados, sin huérfanos y sin faltantes.
3. **Una frontera para todas las completions:** `completeLoopModel` ejecuta `assertLoopMessagesSequence` antes de cada `model.complete` del loop. Los cinco call sites vivos —round, composición diferida, forced, reparación monetaria y explicación de registro— atraviesan esa frontera.
4. **Fallo tipado y acotado:** una violación arroja `LoopMessagesSequenceError` con code fijo `KIPU_LOOP_MESSAGE_SEQUENCE_INVALID`, índice y rol cerrado; no contiene mensajes, argumentos ni datos del usuario.
5. **Auditoría de los otros refresh:** resume agrega el refresh antes de abrir cualquier completion y parte de mensajes reconstruidos sin tool calls abiertos; el diferido agrega el tool result placeholder al clasificar cada call y sólo refresca al cerrar el conjunto. Ambos órdenes son válidos por construcción.
6. **Red de clase:** IR340 fija el orden correcto, rechaza intercalado y faltante, comprueba el error tipado y prueba que no queden completions directas. M0M533 reordena el confirm y M0M534 desactiva el assert; ambos mueren por IR340.
7. **Resultado:** M117 3/3, dry MOCK 20/20 con residuo cero, tsc/lint/build verdes, capture 857/857, mutaciones 528/528 solas y PostgreSQL 82/82.

## 2. Archivos creados o modificados en 1AD

- `src/lib/ai/agent/kipu-agent-loop.ts` — validador puro, error tipado, wrapper único de completions y orden del confirm corregido.
- `src/app/dev/capture-test/page.tsx` — IR340 conductual y smoke de wiring en ambos sentidos.
- `scripts/qa/telegram-agent-regression-audit.mjs` — M0M533 y M0M534.
- `scripts/qa/README.md` — contrato de secuencia de mensajes del loop.
- `docs/M0_LOOP_ETAPA_3L_REPORT_2026-08-16.md` — este reporte.

## 3. Decisiones de diseño y evidencia

### 3.1 El validador modela ids, no el orden accidental del batch

Cuando un assistant abre N tool calls, el estado pendiente es el conjunto exacto de sus ids. Las N respuestas siguientes deben ser mensajes `tool` con ids distintos pertenecientes a ese conjunto; pueden llegar en cualquier orden, pero ningún `system`, `user`, nuevo `assistant`, id repetido o tool huérfano puede aparecer antes de cerrarlo. Esto prueba la condición del proveedor sin imponer un orden más estrecho que el contrato.

### 3.2 La validación vive inmediatamente antes de pagar

No se agregó un check disperso por branch. Los cinco sitios de completion llaman a `completeLoopModel`, que valida el array final que realmente se enviará y sólo entonces delega a `model.complete`. Así también cubre mensajes adicionales construidos para composición o reparación.

El error contiene únicamente:

~~~text
KIPU_LOOP_MESSAGE_SEQUENCE_INVALID index=<entero> role=<assistant|tool|system|user|unknown|end>
~~~

El `code`, `index` y `role` también viven como campos tipados del error. Un request inválido falla localmente y nunca consume una llamada del proveedor.

### 3.3 Posición de los tres refresh auditada

- **Confirmación:** ejecuta el lote, marca `manifestExecuting`, anexa el tool result del confirm y recién después agrega el refresh. Es el único sitio corregido.
- **Resume:** reconstruye mensajes sin `tool_calls`, ejecuta los steps restantes, agrega un refresh, settle y receipts recuperados; no existe assistant abierto.
- **Diferido:** cada tool call recibe primero su tool result placeholder. La resolución económica y el refresh ocurren sólo cuando la completion ya fue clasificada completa; la composición posterior pasa por el validador.

No cambió `settleDurableWork`, ningún writer, executor, SQL ni el camino envelope/`KIPU_AGENT_MODE=on`.

### 3.4 Red y cardinales

- Capture anterior: 856. IR340 añade una aserción: **856 + 1 = 857**.
- Mutaciones anteriores: 526. M0M533–534 añaden dos: **526 + 2 = 528**.
- Dry-run: conserva las **20** patas, incluidas cohesión, redirects, sucesor pay+close y lectura post-confirm.
- IR340 prueba una secuencia válida de dos tools, el intercalado `system`, la respuesta faltante, el error local con índice/rol, el orden exacto del confirm, las posiciones de resume/diferido y la ausencia de completions que evadan el wrapper.

### 3.5 Verificación previa y alcance factual

Se leyó completa la ADENDA 25 y se verificó en el código la secuencia defectuosa descrita: el branch de confirm tenía el refresh inmediatamente antes de su tool result. El cambio invierte únicamente esas dos operaciones posteriores al lote. Los otros call sites no reproducían la clase y quedaron documentados, no modificados.

## 4. Salidas íntegras de gates

Los gates se ejecutaron en el orden contractual. El servidor loop se detuvo antes de tsc/lint/capture/build; mutaciones corrió sola y PostgreSQL comenzó únicamente después de su exit 0. Todos los comandos conservaron su exit code directo, sin pipes.
### 4.1 M117 post-aplicación

Comando: `node --env-file=.env.local ./scripts/qa/m0-loop-117-e2e.mjs`

~~~text
(node:47517) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
...
[kipu.route] {"ts":"2026-08-16T21:49:40.541Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":false,"transactionType":"fixed_expense_create"}
[kipu.route] {"ts":"2026-08-16T21:49:40.541Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":true,"transactionType":"fixed_expense_create"}
[kipu.route] {"ts":"2026-08-16T21:49:40.541Z","route":"commitment","outcome":"fixed_expense_clarification","dbWrite":false,"transactionType":"fixed_expense_create"}
857/857 capture checks
(node:47982) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/capture/capture-matching.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
~~~

Exit: 0. Resultado: 857/857.
...
✓ Generating static pages using 11 workers (36/36) in 218ms
...
M0 mutations: 528/528
~~~

Exit: 0. Resultado: 528/528; la auditoría corrió sola. M0M533 y M0M534 murieron por IR340; cero crashes y cero anchors distintos de uno.
...
~~~text
(node:51524) [MODULE_TYPELESS_PACKAGE_JSON] Warning: Module type of file:///Users/nicot/Projects/fincoach-mvp/src/lib/ai/agent/agent-operation-store.ts is not specified and it doesn't parse as CommonJS.
Reparsing as ES module because module syntax was detected. This incurs a performance overhead.
To eliminate this warning, add "type": "module" to /Users/nicot/Projects/fincoach-mvp/package.json.
(Use `node --trace-warnings ...` to show where the warning was created)
...
- No se ejecutó la enfocada ME17 + REAL_FOUR_CREDITS ni ninguna corrida completa.
...
- Queda pendiente la auditoría de Claude y, sólo con su OK, reactivar los dos pasos de A24.

## 6. Riesgos y objeciones

### 6.1 Orden de respuestas tool

El proveedor exige cerrar todos los ids antes de otro rol, pero no exige que las respuestas tool repitan el orden de declaración. El validador acepta cualquier permutación exacta de ids. Endurecerlo a orden posicional rechazaría batches estructuralmente válidos sin ganar seguridad.

### 6.2 Historial reconstruido

Los mensajes históricos que alimentan resume conservan role+content y no tool calls antiguos. Por eso el refresh de resume es legal en el historial vivo. El validador, además, fallaría localmente si una futura reconstrucción comenzara a conservar un assistant abierto sin sus tools.

### 6.3 Diagnóstico acotado

El índice y el rol identifican la frontera rota sin persistir texto libre. No identifican el nombre de la tool ni su payload deliberadamente; añadirlos ampliaría innecesariamente la superficie durable de diagnóstico.

### 6.4 Alcance de la certificación

IR340 y el dry-run prueban la estructura antes del mock provider y todos los gates están verdes. La confirmación de que el proveedor real deja de devolver el 400 corresponde a la enfocada ME17 + REAL_FOUR_CREDITS posterior al OK de Claude, no a esta entrega gratuita.

## 7. DDL propuesto

Ninguno. 1AD es TS/capture/mutaciones/documentación solamente.
