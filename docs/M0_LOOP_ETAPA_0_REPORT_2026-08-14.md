...
# M0 Loop — Reporte de Etapa 0

Fecha: 2026-08-14  
Estado: **LISTA PARA AUDITORÍA DE CLAUDE**  
Base contractual: `docs/M0_CLAUDE_AUDIT_TOOL_LOOP_2026-08-14.md`, §7 Etapa 0.

## 1. Alcance implementado vs. contrato de la etapa

Se implementó todo el alcance ejecutable de Etapa 0:

- Se congeló el frente de envelope: no se ejecutó ninguna muestra pagada, no se desplegó el árbol M0.11A y no se cambió producción.
- El camino de evidencia ya no convierte un fallo tipado del agente en `reply:""`. Si el resultado trae un mensaje publicable, conserva exactamente ese mensaje; si no lo trae, usa la misma continuidad tipada/no-write del chat ordinario.
- Los dos sitios vivos de evidencia —digest inicial y replay de estado de cuenta— pasan por una única función pura y publican una respuesta no vacía antes de volver al webhook.
- El chat web convierte los rechazos del handler en un error accionable de UI. No fabrica una burbuja del asistente cuando no existe texto seguro del agente y reintenta con el mismo `submissionId`.
- Se fijaron ambas fronteras con checks de capture y un mutante específico del silencio de evidencia.
- tsc, lint, build, capture, mutaciones y PostgreSQL ×1 cierran con exit 0.

Desviaciones: ninguna respecto del contrato de Etapa 0. El camino `KIPU_AGENT_MODE=on` no fue modificado como parte de esta etapa.

## 2. Archivos creados/modificados

- `src/lib/capture/evidence-capture.ts`: añade `publishableEvidenceAgentReply` y lo consume en los dos caminos de respuesta del agente de evidencia.
- `src/app/app/transaction-actions.ts`: convierte el rechazo del handler web en `deliveryError` tipado y reintentable, después de autenticar.
- `src/app/app/components/ChatView.tsx`: presenta el error de entrega como estado accionable fuera de la conversación y conserva la identidad de reintento.
- `src/app/dev/capture-test/page.tsx`: fija TG-17 para mensaje seguro/continuidad de evidencia y amplía TG-6 para la UI web.
- `scripts/qa/telegram-agent-regression-audit.mjs`: M0M147 vuelve a matar la continuidad de evidencia y prueba que TG-17 lo detecta.
- `docs/M0_LOOP_ETAPA_0_REPORT_2026-08-14.md`: este reporte.

## 3. Decisiones de diseño

1. **Una sola política de publicación para evidencia.** La función pura prioriza `agentResult.message?.trim()` y cae en `antiBotContinuityReply`. Así no inventa una segunda voz ni duplica la lógica de continuidad ya auditada.
2. **El estado de evidencia sigue siendo veraz.** Publicar continuidad no convierte un fallo en éxito: `ok`, `status` y `retryable` siguen derivándose del resultado real. Solo se elimina el silencio del transporte.
3. **Un rechazo de entrega no es autoría del asistente.** El chat web usa estado de UI con botón de reintento, una de las dos salidas admitidas por §7. No agrega el mensaje accionable al transcript como si Kipu lo hubiera producido.
4. **La identidad se conserva.** El botón reutiliza exactamente el `submissionId` original; los writers y la delivery durable conservan su frontera de dedupe.
5. **Autenticación fuera del catch.** El `redirect("/login")` queda antes del bloque que normaliza fallos de entrega, para no degradar una sesión ausente a un falso error reintentable.

## 4. Salida íntegra de los gates

No hubo llamadas de modelo ni muestras pagadas.
...
- No se tocó el comportamiento de `KIPU_AGENT_MODE=on`, el planner, la publicación v44 ni cuerpos de executors.
- No se creó, aplicó ni modificó ninguna migración.
- No se hizo commit, push ni deploy.
- No se ejecutó la batería conversacional con modelo.
- La Etapa 1 se reporta por separado porque su verificación previa encontró un bloqueo contractual en las RPC aplicadas.

## 6. Riesgos y objeciones

1. `sendChatMessageAndGetReply` colapsa deliberadamente los throws posteriores a autenticación en un único `chat-delivery-rejected`. Es seguro para el usuario y accionable, pero pierde clasificación diagnóstica en esa respuesta de Server Function. No se amplió el alcance para cambiar el contrato del handler o persistir nuevos diagnósticos.
2. La continuidad de evidencia confía en que un `message` no vacío de `runKipuAgent` ya atravesó sus barreras de publicación. Es la misma autoridad que usa el chat ordinario; añadir una segunda sanitización aquí podría alterar texto legítimo o divergir entre transportes.
3. El build mantiene una advertencia preexistente de NFT/Turbopack y lint mantiene la advertencia de Babel por el tamaño de `capture-test/page.tsx`. Ninguna fue introducida por el quick-win y ambas cierran con exit 0.

## 7. DDL propuesto

Ninguno para Etapa 0.
