...
**Estado: CERRADO (2026-08-24).** Migraciones 100–124 aplicadas. El expediente
completo de implementación, uso real y auditorías vive en
`docs/M0_CLAUDE_AUDIT_TOOL_LOOP_2026-08-14.md` y los informes históricos
`docs/M0_*`.

M0 terminó con un único cerebro de producción: el loop nativo de tool-calling
en `kipu-agent-loop.ts`. El modelo interpreta la conversación y llama tools;
los writers tipados, el manifiesto durable de operación, PostgreSQL, CAS,
verificación y replay conservan la autoridad sobre el dinero. El pipeline
legacy queda congelado únicamente como rollback con `KIPU_AGENT_MODE=off`.
`AgentMode` es `off | loop`; los valores históricos `on`/`shadow` son
aliases de compatibilidad a `loop` con aviso único.

El planner envelope, sus validadores/compiladores, las barreras de publicación
v29–v44, el recovery de intake de ese camino y su harness de modelo fueron
eliminados por inalcanzabilidad. No cambió ninguna superficie viva: el loop,
...
entidad, montos almacenados, confirmación multiacción, continuidad, memoria,
calendario, receipts/post-write y voz/anti-loop, entre otras.

**Orden vivo:** Bloque M es el único bloque activo. No queda otro stage de back
por delante del front.
...
| **N3C · El orbe de ElevenLabs** | *(tercer acto, 2026-08-29)* El founder puntuo N3B en 4,5 y trajo la referencia exacta: los orbes de ElevenLabs. Verificado leyendo su fuente (MIT): NO es 3D, es un disco con shader — el look vive en el shader, no en three. Se adopta su look con nuestro liquido adentro, nivel y aire, y la onda de voz sobre la superficie |
...
096–099 sigue siendo 40/40 con exit 0 y residuo cero. La continuidad, comprensión y ejecución general del agente quedaron cerradas
en M0; esa red permanente no invalida ni reemplaza la integridad monetaria
probada por Pre-M.
