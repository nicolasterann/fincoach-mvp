...
structured context, calls the complete typed tool surface, and uses the durable
operation manifest (112–115) for exact multi-action authorization, CAS,
verification and replay. The retired envelope planner, its bounded-repair and
publication stack, and its model harness were deleted because they were
unreachable from both live modes. `AgentMode` is now `off | loop`; old
...
reports. Migrations 001–124 are applied; the next migration is 125.
...
- The brain is an **LLM agent** that interprets intent broadly and chooses
  **tools**. Add capabilities as **tools**, never as new regex routes or
  phrase gates.
- Production posture: `KIPU_AGENT_MODE=loop` — the native tool-calling loop is
  the primary brain; the frozen legacy pipeline is emergency rollback only
  (never re-extend its gates). The tool surface is ~124 typed tools. Agent,
  chat, ambient, and fallback
...
(`request_text` nullable, filas depuradas, recorder sin persistirlo). The full
  applied chain is 001–124; the next migration is 125.
