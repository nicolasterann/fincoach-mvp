...
export async function buildUserFinancialContext(
  ...
  const [
    ...
    supabase
      ...
      .select(
        "id, user_id, name, target_amount, currency, current_amount, target_date, goal_account_id, funding_account_id, status, feasibility_status, weekly_required_amount, monthly_required_amount, notes, archetype, goal_type, contribution_amount, cadence, cashflow_protected, created_at",
      )
      .eq("user_id", userId)
      .order("created_at", { ascending: true })
      .limit(CONTEXT_CAP + 1),
