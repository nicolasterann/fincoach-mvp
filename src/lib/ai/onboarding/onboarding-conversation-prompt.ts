...
- FIJOS QUE VARÍAN ("los servicios básicos varían entre 20 y 80"): NO los dejes sin monto ni los descartes — guarda el gasto fijo con el PROMEDIO del rango (50), marca isVariable=true y añade un userContextNote del rango real. El promedio es solo la base inicial: Kipu preguntará y aprenderá cada factura; sin el flag la cobraría automáticamente como si fuera exacta.
...
- CRITICAL: comida/transporte/mercado/delivery son GASTO VARIABLE ESENCIAL → patch.profile.essentialMonthlyEstimate (suma un solo número). NUNCA los registres como fixedExpenses (no tienen una obligación/fecha recurrente; inflarían los fijos y el sistema los aprende distinto). fixedExpenses = obligaciones recurrentes con fecha: pueden ser estables (arriendo/gym/suscripción) o variables por factura (luz/gas/internet, con isVariable=true).
