...
export default function Home() {
  ...
  const biggestBudgetGap = dashboard.budgetReality.biggestGap;
      ...
      <section className="mx-auto flex w-full max-w-md flex-col gap-5">
        ...
        <section className="rounded-3xl bg-white p-5 text-zinc-950 shadow-2xl">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="rounded-full bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-800">
              {dashboard.goalFeasibility.status === "challenging"
                ? "Desafiante"
                : "Activa"}
            </div>
          </div>
        ...
        <section className="rounded-3xl bg-white/10 p-5">
          ...
          <p className="mt-3 text-sm leading-6 text-zinc-300">
            {biggestBudgetGap ? (
              <>
                Pensábamos que {biggestBudgetGap.category} estaba cerca de{" "}
                <strong>{formatMoney(biggestBudgetGap.initialEstimate, "USD")}</strong>,
                pero tu realidad va más cerca de{" "}
                <strong>{formatMoney(biggestBudgetGap.currentEstimate, "USD")}</strong>.
                No pasa nada. Ahora podemos planear con números reales.
              </>
            ) : (
              "Todavía estamos aprendiendo tu realidad financiera."
            )}
          </p>
        </section>
...
function MetricCard({
  ...
  return (
    <article className="rounded-3xl bg-white p-4 text-zinc-950 shadow-xl">
      <p className="text-xs foxt-zinc-500">{label}</p>
